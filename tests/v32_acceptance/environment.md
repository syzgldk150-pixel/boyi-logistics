...
浏览器库的持久准备入口是 `python -m tests.v32_acceptance.prepare_database`，默认只迁移，显式加 `--reset-owned-fixture` 才重建。完整验收驱动在页面段开始时自动使用该显式重置，再依次执行 `tests.v32_acceptance.management_fixture`、`tests.v32_acceptance.data_fixture`、`tests.v32_acceptance.browser_performance`、`tests.v32_acceptance.detail_browser`、`tests.v32_acceptance.navigation_probe`、`tests.v32_acceptance.run_acceptance`（均使用 `python -m`）；独占锁会拒绝仍有参与者的重置。简单设置实际生效入口为 `python -m tests.v32_acceptance.settings_effective`，要求明确 `--database v32_m01_test`；通过真实页面保存参数和账号，执行下一次插件，再从页面恢复原历史设置并再次执行。
