...
def test_first_party_descriptors_are_16_actions_and_18_instances(
    ...
    assert manifests["sync_scan_codes"].version == "1.0.25"
    assert manifests["sync_site_send_list"].version == "1.0.21"
    ...
    assert manifests["sync_daily_send_orders"].version == "1.0.23"
    ...
    assert {seed.version for seed in seeds if seed.plugin_id == "sync_arrive_list"} == {"1.0.21"}
    assert {
        seed.version
        for seed in seeds
