...
def direct_repository():
    ...
    helper._apply_through(helper.database, "044")
...
class DirectFixture:
    def __init__(self, management, *, directory: Path | None = None, handlers=None, account_manager=None, saved_resource_provider=None):
        ...
        self.broker = LocalCoreAutomationBroker(issuer=self.issuer, adapter=RegisteredCoreAutomationBrokerAdapter(handlers=management.broker_handlers if handlers is None else handlers, account_resolver=AccountManagerSessionResolver(account_manager or management.account_manager), resource_resolver=management.binding_resolver))
