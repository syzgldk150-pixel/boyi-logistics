...
class MySqlOrchestrationIntegrationTests(unittest.TestCase):
    ...
    def test_feishu_notification_lease_holds_the_database_binding_lane(self):
        """Migration 030 keeps a terminal in-flight send ahead of its queue."""

        repository = self._repository()

        def create_approval(label: str) -> tuple[str, str]:
            command, item, run, event, outbox = self._aggregate_rows(label)
            with repository.unit_of_work() as uow:
        ...
        with self._connection(autocommit=True) as connection, connection.cursor() as cursor:
            ...
            self.assertIn("VIRTUAL GENERATED", str(generated["EXTRA"]).upper())
            self.assertIn(
                "NOTIFICATION_LEASE_TOKEN",
            ...
            self.assertEqual(0, int(cursor.fetchone()["NON_UNIQUE"]))
            cursor.execute(
                """
                SELECT CHECK_CLAUSE
            ...
            with self.assertRaises(self.pymysql.err.IntegrityError):
                cursor.execute(
            ...
            with self.assertRaises(
                (self.pymysql.err.IntegrityError, self.pymysql.err.OperationalError)
            ):
                cursor.execute(
                    "UPDATE feishu_approval_deliveries "
            ...
            self.assertEqual(1, int(cursor.rowcount))
