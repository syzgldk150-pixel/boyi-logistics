...
class RuntimeRepositoryTests(unittest.TestCase):
    ...
    def test_migration_runner_discovers_ordered_sql_without_loading_configuration(self):
        ...
        self.assertEqual(
            ["050_normalize_local_waybill_dates.sql", "051_separate_boyi_waybills.sql", "052_problem_write_intents.sql"],
            [path.name for version, path in migrations if version in {"050", "051", "052"}],
        )
