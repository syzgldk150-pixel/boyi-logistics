...
- **分批差错及问题件**：飞书文本只接受精确“分批”。首次 dry-run 后按每日到货表顺序连续编号，支持单号序号、多选、区间、中文分隔符和“全部”；选中后进入第二个 10 分钟确认状态。重复、重叠、越界或非法序号显式报错并保留当前列表。正式执行携带所选运单与预览指纹，登录恢复后仍须重校验。旧文本只提示发送“分批”，菜单事件不直接执行该工具。
