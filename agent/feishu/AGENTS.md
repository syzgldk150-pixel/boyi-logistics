...
- 用户回"是" → 调 `POST /internal/v1/admin/tms/session/send-code` → 注册 `waiting_code_for_resume`
- 用户回 4-8 位字母数字验证码 → 调 `POST /internal/v1/admin/tms/session/submit-code` → 成功后自动续跑原任务
...
- 大祥账号走 `/internal/v1/admin/tms/price-session/send-code`，账号密码来自 `TMS_DAXIANGUSERNAME` 等环境变量
- 操作场账号走 `/internal/v1/admin/tms/session/send-code`，后台输入的密码仅保存在当前 Agent 进程内存
...
- 融辉报价发码/提码接口走 `/internal/v1/admin/tms/price-session/*`
- 韵达报价登录恢复走 `/internal/v1/admin/tms/yunda-session/*`
- 其他 TMS 工具继续走 `/internal/v1/admin/tms/session/*`
