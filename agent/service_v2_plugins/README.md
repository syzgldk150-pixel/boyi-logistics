...
当前修订版为打卡包 `1.2.3`、寄件同步包 `2.0.3`、分批问题件包 `2.0.2`、其他包 `2.0.1`：对象/列表字段可正常加载，多账号选择保持完整集合。寄件同步按日期列语义核验飞书回读，同时保留其他字段的完整比对。版本号属于各插件自身，均使用 Service V2 协议；不是 V1 运行器。真实 ZIP 设置页浏览器回归见 `tests/test_production_plugin_settings_browser.py`。历史 `2.0.0` 清单仅作为迁移 048 的隔离测试数据保留，不参与运行或打包。
