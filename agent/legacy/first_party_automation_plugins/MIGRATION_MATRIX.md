...
| `sync_arrive_list` | `tools/arrive_list_sync_tool.py` | `account_id` + `arrive_primary_sheet` / `arrive_secondary_sheet` resources | `browser.invoke/ronghui.arrive_list.read_page`; `projection.invoke/arrival.report.publication.read`, `waybill.snapshot.replace`, `arrival.forecast_snapshot.replace`; `network.request/feishu.sheet.replace` | `RUNNABLE`; package owns pagination, normalization, `Decimal` conversion, filtering and commit order. Version 1.0.21 reads both report publications before any write: same-day statistics with the same source account and exact physical Sheet retain ownership, while the current MySQL list and forecast are refreshed. Damaged/rebound publication or overwritten statistics requires a new statistics run; a fresh successful publication replaces the earlier version. The host never reconstructs rows from history. MySQL and every actual Sheet write still require independent exact readback. Signed ZIP → Broker → Router → ResultVerifier and real-MySQL list → statistics → list → next-day list tests cover the boundary without account/resource IDs in plugin JSON or a whole-tool fallback. |
