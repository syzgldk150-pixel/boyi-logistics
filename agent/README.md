...
- `price_scripts/`
  价格工具底层业务目录
- `../shared/finance/`
  财务领域模型、金额规则、校验、费用映射与共享账本仓储
- `agent/tms_runtime/scripts/*finance*`、`tools/finance_sync_service.py`
  融辉真实财务页面采集与多账号同步编排；韵达财务适配器保留但未进入生产调度
...
- 然后默认打开首页大盘 `http://127.0.0.1:8765/`；需要验证业务账号登录态时进入 `/automation-accounts`
- 凭据和登录态只在“业务账号”模块管理；`/automations` 项目卡只保存当前账号角色绑定，不回显凭据，也不选择默认或首项账号
- 项目设置保存成功后会显示明确反馈并自动收起设置面板；账号改绑在下一次运行生效
