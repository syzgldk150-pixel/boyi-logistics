...
_MANAGED_CONTRIBUTION_KINDS = _SERVICE_V2_CONTRIBUTION_KINDS[1:]
...
class ManagedContributionRegistry:
    ...
    def register(self, material: Mapping[str, Any], *, committed: bool) -> None:
        ...
        candidate = self._from_material(material, phase=phase)
        with self._lock:
            existing = self._registrations.get(candidate.registration_id)
            if existing is not None:
                ...
                if committed and existing.phase != "COMMITTED":
                    self._registrations[candidate.registration_id] = replace(
                        existing,
                        phase="COMMITTED",
                    )
                return
            for route_key in candidate.route_keys:
                for owner_id in self._route_owners.get(route_key, ()):
                    owner = self._registrations[owner_id]
                    if (
            ...
            self._registrations[candidate.registration_id] = candidate
            for route_key in candidate.route_keys:
                self._route_owners.setdefault(route_key, set()).add(
                    candidate.registration_id
                )

    def activate(self, automation_id: str, generation: int) -> None:
        with self._lock:
            for registration_id, record in tuple(self._registrations.items()):
                if record.automation_id != automation_id:
                    continue
                phase = "COMMITTED" if record.generation == generation else "DRAINING"
                if record.phase != phase:
                    self._registrations[registration_id] = replace(record, phase=phase)

    def unregister(self, registration_id: str) -> None:
        with self._lock:
            record = self._registrations.pop(str(registration_id), None)
            if record is None:
                return
            for route_key in record.route_keys:
                owners = self._route_owners.get(route_key)
                if owners is None:
                    continue
                owners.discard(record.registration_id)
                if not owners:
                    self._route_owners.pop(route_key, None)
