...
DEFAULT_LOGIN_USER = os.getenv("TMS_DAXIANGUSERNAME", "").strip()
DEFAULT_LOGIN_PASS = os.getenv("TMS_DAXIANGPASSWORD", "").strip()

MODULE_DIR = os.path.dirname(os.path.abspath(__file__))
_BROWSER_RESOLVER = None
_BROWSER_RESOLVER_KEY = ""
...
ADDRESS_RESOLVE_ERROR_CODE = "RONGHUI_ADDRESS_RESOLVE_FAILED"
UNREACHABLE_RESOLUTION_ERRORS = (
    "destination paused for delivery",
    "destination not available",
)


def _is_unreachable_resolution_error(error_text: str) -> bool:
    normalized = _clean_str(error_text)
    return any(marker in normalized for marker in UNREACHABLE_RESOLUTION_ERRORS)


def _load_browser_resolver_class():
    if __package__:
        from agent.tms_runtime.scripts.browser_address_resolver import BrowserAddressResolver

        return BrowserAddressResolver

    module_path = os.path.join(MODULE_DIR, "browser_address_resolver.py")
    module_name = "_ronghui_runtime_browser_address_resolver"
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"could not load browser address resolver from {module_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.BrowserAddressResolver


def _get_browser_resolver(config_path: Optional[str] = None):
    global _BROWSER_RESOLVER, _BROWSER_RESOLVER_KEY
    config_key = os.path.abspath(config_path) if config_path else ""
    if _BROWSER_RESOLVER is not None and _BROWSER_RESOLVER_KEY == config_key:
    ...
    with _BROWSER_RESOLVER_LOCK:
        ...
        if _BROWSER_RESOLVER is not None:
            try:
                _BROWSER_RESOLVER.close()
            except Exception:
                pass
        BrowserAddressResolver = _load_browser_resolver_class()

        _BROWSER_RESOLVER = BrowserAddressResolver(
            username=DEFAULT_LOGIN_USER,
            password=DEFAULT_LOGIN_PASS,
            config_path=config_path or "",
        )
        _BROWSER_RESOLVER_KEY = config_key
    return _BROWSER_RESOLVER
...
def _resolve_destination_via_browser(
    ...
    *,
    ...
    try:
        browser_result = _get_browser_resolver(config_path).resolve(address)
...
def resolve_address_destination(
    ...
    *,
    ...
    return _resolve_destination_via_browser(session, original_address, config_path=config_path)
...
def _calc_sum_fee(calc_data: Dict[str, Any]) -> Decimal:
    total = Decimal("0.00")
...
def _build_base_payload(
    ...
    return {
        ...
        "CITY_NAME": f"{province}-{city}",
        "COUNTY_NAME": f"{city}-{accept_county}",
        "TOWN_NAME": f"{city}-{accept_county}-{town}",
...
def fetch_prices(address: str, weight: float, volume: float, config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _unreachable(reason: str = "") -> Dict[str, str]:
        payload = {"网点不可达": "网点不可达"}
        if reason:
            payload["address_resolution_error"] = reason
        return payload

    def _address_resolution_failure(reason: str) -> Dict[str, str]:
        logging.warning("ronghui address resolution failed: %s", reason)
        return {
            "error": f"融辉地址解析失败：{reason}",
            "error_code": ADDRESS_RESOLVE_ERROR_CODE,
            "address_resolution_error": reason,
            "mode": "agent_tms",
        }

    try:
        resolved = resolve_address_destination(
            session,
            address,
            ctx,
            emp_code,
        ...
        reason = _clean_str(exc)
        if _is_unreachable_resolution_error(reason):
            return _unreachable(reason)
        return _address_resolution_failure(reason)
