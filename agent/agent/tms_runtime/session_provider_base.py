    ...
    def _send_yunda_sms_code_locked(self, page: Any) -> None:
        try:
            ...
            if not send_button.is_visible(timeout=2_000):
                raise TMSAuthStateError(
                    "AUTH_REQUIRED", "韵达短信发送按钮不可见，请重新登录韵达账号。"
                )
            if not send_button.is_enabled(timeout=2_000):
                raise TMSAuthStateError(
                    "AUTH_REQUIRED", "韵达短信发送按钮当前不可用，请稍后重试。"
                )
        except TMSAuthStateError:
            raise
        except Exception as exc:
            ...
            raise TMSAuthStateError(
                "AUTH_REQUIRED", "无法确认韵达短信发送按钮状态，请重新登录韵达账号。"
            ) from exc

        try:
            with page.expect_response(
                lambda response: (
                    str(getattr(getattr(response, "request", None), "method", "")).upper()
                    == "POST"
