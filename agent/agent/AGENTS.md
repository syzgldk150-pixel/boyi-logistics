...
- 改 Service v2 动态飞书命令：
  ...
  - `sync_scan_codes` 的未知写恢复由 `plugin_core_adapters/first_party.py` 有界枚举当前及已归档 generation 的未知 lease，再读取 `production.py` 基于正式 lease、Command 与已验签 preview 构造的服务端上下文；只有融辉发件扫描账本在 naive UTC 与 naive 上海时间两种历史解释对应的全部精确时间窗内都没有任何目标单号/下一站记录时，才以 receipt 身份证明 `NOT_APPLIED`。发布生成 successor generation 不得让旧未知 lease 脱离恢复范围；部分、重复、非空不闭合或来源异常不得自动释放互斥。
