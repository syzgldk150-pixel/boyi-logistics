...
- `scripts/automation_project_resource_preflight.py` 封装迁移 018 的八项 required-existing 资源只读前检；`scripts/automation_project_schedule_identity_preflight.py` 从共享迁移清单构造 70 项计划任务身份并仅在 018 待执行时做只读前检；`run_migrations.py` 仅通过脚本同目录 exact-path loader 绑定这些公开检查函数，禁止裸导入或修改全局 `sys.path`。
