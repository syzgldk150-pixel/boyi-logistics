...
- 必须显式解析独立的 R13 来源账号和唯一的融辉 TMS 邵阳大祥站 `account_id`；R13 查询站点只由自动化项目绑定的 `r13_account_id` 对应中央账号目录合同解析，且必须精确匹配账号合同常量 `DAILY_SIGN_R13_SITE_CODE=7390017`，其他非空站点同样拒绝。业务脚本与请求体不得硬编码、猜测或覆盖站点。同一个 TMS 登录态统一用于问题件、主单签收、轨迹核验和地址补全，不读取旧 `phase7.r13_credentials`，不接受内联凭据、隐式账号或多候选。
- 长历史签收按 31 天窗口分片并校验总量；离开当前 R13 的候选由迁移 `013` 的状态按 1/3/7 天退避精确复核。来源不完整或冲突无法核验必须显式阻塞；R13 完整零行仍需完成其他证据核验，若最终发布集合为空且飞书仍有上一版数据，则投影层显式阻塞并保留上一成功投影，目标本就为空且证据闭合时可接受合法零集合。
