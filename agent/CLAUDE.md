...
- `scripts/automation_project_resource_preflight.py` 封装迁移 018 的八项 required-existing 资源只读前检；`run_migrations.py` 仅通过脚本同目录 exact-path loader 绑定其公开检查函数，禁止裸导入或修改全局 `sys.path`。
