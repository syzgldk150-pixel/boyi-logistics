...
- 审查修复后，`service_v2_process_runtime.py` 是 Harness 与 Webhook/Event 进程 backend 的唯一 composition boundary：`HarnessRuntime` 只在真实 Bubblewrap + `prlimit` canary 成功后投影 `READY/OFFLINE_RESTRICTED`，公共 Catalog 与 sidecar 执行共用同一 active snapshot，仅接受 `调用只读工具：<完整标题>` 的唯一精确零参数动态工具，无 handler 的六类固定工具不入 Catalog。`ServiceV2ManagedIngress` 只公开已验证 Webhook/Event identity 的两个进程内调用，持久注册与当前绑定同时有效才是 `READY`，未绑定返回 `PROJECT_RUNTIME_PROJECTION_STALE`，不新增 HTTP 路由。tracking fixture 只能由显式 `connector-test` 命令通过真实 `ConnectorRegistry.invoke` 加载，状态为 `offline_contract=COMPLETE`、`offline_runtime=READY`、`production_runtime=PRODUCTION_GATED`。真实 LLM/业务读网关、公网 Webhook/外部 Event 源、真实 Connector 和生产数据仍为 `PRODUCTION_GATED`。
...
- Business Account 池与 `workflow_resources` 资源池进入插件目录前只能投影闭合的安全 descriptor；资源固定为 `resource_id/name/kind/status`，不得把 Token、表格 ID、读写范围、文件路径、配置哈希/版本或原始配置送入浏览器。飞书资源的 `name` 必须由服务端按当前文档名与工作表名实时解析并短时缓存，改名后自动刷新；权限、定位或元数据不完整时资源池整体 fail closed，禁止回退静态业务别名或内部资源 ID。插件目录的 `hidden_automation_ids` 只投影真实持久化且当前发行明确排除的身份，不得用它生成静态项目；Console 只能按签名清单声明的 role 与 kind 精确筛选并保存 ID，不默认选择第一项；所有签名项目自动化运行只接受项目当前提交的精确账号绑定，后台改绑后下一次运行使用新 ID，脚本、Broker、调度器均不得按 `is_default`、列表顺序或固定账号补齐；池不可用、descriptor 漂移、必填绑定缺失/停用/类型不符时，配置、运行、启用和完全自动均 fail closed。
