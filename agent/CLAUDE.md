...
- 自提问题件必须读取大祥到货数据表中已审阅的 `UeBd3I` 工作表；分批/未到问题件继续读取其独立来源。两者保留独立资源角色，启动同步必须把自提资源的持久化定位恢复为 `UeBd3I`；发布启动只把历史遗留的 `phase7.arrive_primary_sheet` 项目绑定迁移到 `phase7.self_pickup_source_sheet`，已正确或管理员自定义的资源绑定不得覆盖，禁止再次跟随分批来源或按同名工作表猜测。
