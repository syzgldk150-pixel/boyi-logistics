...
- Console 的“核验并恢复”仅用于闭合历史未知 receipt，不是重新执行入口；浏览器只提交请求 UUID，Agent 只依据持久 receipt 与权威回读判定 `WRITE_VERIFIED`、`NOT_APPLIED` 或 `UNKNOWN`，不得接受浏览器提交 generation、lease 或 evidence，也不得重放原 Run。`sync_scan_codes` 的新触发可在接受前有界枚举当前及已归档 generation 的旧未知 lease，按旧正式 Command 绑定的已验签预览精确恢复单号/下一站；缺少新版紧凑预览上下文的历史 Command 必须先验证同一 Run 的完整 Plan hash，再从 Plan impact 恢复并逐字段匹配原完成 Step，不得依赖历史内层 fingerprint 的重新序列化。回读同时覆盖旧版 naive UTC 与 naive 上海时间解释查询融辉发件扫描账本；所有精确身份在全部时间窗都为零时才落 `NOT_APPLIED` 并接受全新 Command，部分、重复、登录失败、结构异常或非零不闭合结果继续保持 `UNKNOWN`。其他普通重新执行直接提交新 Command，不得把未知写当作已取消。
