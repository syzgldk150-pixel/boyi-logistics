...
- Console 的“核验并恢复”只提交浏览器请求 UUID；Agent 必须从当前 committed generation 服务端选择唯一一条 unresolved unknown-write lease，并只依据持久 receipt 事务恢复。候选缺失、多条、证据不完整或运行态不闭合都返回 `UNKNOWN`，保持隔离且不得重放；浏览器提交 generation、lease 或 evidence 必须拒绝。
