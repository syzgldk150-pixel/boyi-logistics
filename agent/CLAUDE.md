...
- 同一 `automation_id` 实行项目级 single-flight：Command 接受事务先锁定项目与配置，再检查该项目的非终态 Run；不同请求命中时返回 `AUTOMATION_ALREADY_RUNNING` 且不创建第二个 Run，同一幂等请求仍复用原 Command。WorkflowRunner 默认运行 4 个有界 Worker，浏览器工具最多 3 个并发，分别可由 `WORKFLOW_RUNNER_CONCURRENCY` 和 `WORKFLOW_BROWSER_CONCURRENCY` 覆盖；浏览器步骤按精确账号会话加锁，同账号以及未绑定账号的会话串行，不同账号可受控并行。只读/计算工具不使用写互斥锁，受保护写仍只按“账号 + 外部目标 + 资源 + 写类型”精确串行。锁身份不完整时显式阻断，禁止按工具名或 heavy 类别猜测锁。Run 对外投影排队、读取、处理、写入、核验与完成阶段，实际阶段计时只从取得通道后开始。工具子进程、取消和 generation lease 均以 Run + Step 精确绑定。
