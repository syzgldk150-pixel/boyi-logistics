...
def _chrome_executable_path() -> Optional[str]:
    candidates = [
...
def _load_default_credentials() -> Dict[str, str]:
    if not os.path.exists(CONFIG_FILE):
        return {}
    with open(CONFIG_FILE, "r", encoding="utf-8") as f:
        data = json.load(f)
    return data.get("test_user_data") or {}
...
class BrowserAddressResolver:
    def __init__(
        ...
        *,
        ...
        self._frame = None
        self._ocr = None
        self._worker = ThreadPoolExecutor(max_workers=1, thread_name_prefix="ronghui-legacy-address-resolver")
    ...
    def _login_and_open_tab(self) -> None:
        ...
        username = self.username or creds.get("operator_uid", "")
        password = self.password or creds.get("operator_password", "")
        if not username or not password:
            raise RuntimeError("browser resolver missing TMS credentials")
        ...
        self._context = self._browser.new_context(viewport={"width": 1600, "height": 1200})
        ...
        for _ in range(6):
            code = self._ocr.classification(self._page.locator("#yzm_img").screenshot()).strip()
            self._page.fill("#validateCode", code)
            self._page.locator("a.supplier").first.click()
            try:
                self._page.wait_for_url("**/module/index?mv=index", timeout=8000)
                break
            except PwTimeout:
                self._page.locator("#yzm_img").click()
                self._page.wait_for_timeout(500)
        else:
            raise RuntimeError("browser resolver login failed")
    ...
    def _load_page_user_info(self) -> Dict[str, str]:
        sanitized = self._sanitize_user_info(self._load_page_user_info_from_context())
