...
def has_trailing_poi_notes(address: str) -> bool:
    text = normalize_address_text(address)
    return bool(text) and strip_trailing_poi_notes(text) != text
...
def _admin_tokens(province: str, city: str, district: str) -> List[str]:
    tokens = []
    for token in (normalize_address_text(province), _display_city(province, city), normalize_address_text(district)):
...
def has_redundant_admin_prefix(address: str, province: str = "", city: str = "", district: str = "") -> bool:
    cleaned = strip_trailing_poi_notes(address)
    normalized = normalize_address_text(cleaned)
    if not normalized:
        return False

    standardized = build_admin_prefixed_address(cleaned, province, city, district)
    stripped_body = _strip_leading_admin_tokens(cleaned, province, city, district)
    return standardized != normalized and stripped_body != normalized


def has_leading_area_noise(address: str, province: str = "", city: str = "", district: str = "") -> bool:
    cleaned = strip_trailing_poi_notes(address)
    normalized = normalize_address_text(cleaned)
    if not normalized:
        return False
    body = _strip_leading_admin_tokens(cleaned, province, city, district)
    refined = refine_street_body(cleaned, province, city, district)
    return bool(body) and refined != normalize_address_text(body)
...
def preferred_stored_address(address: str, province: str = "", city: str = "", district: str = "") -> str:
    candidates = generate_address_candidates(address, province=province, city=city, district=district)
    return candidates[0] if candidates else normalize_address_text(address)
