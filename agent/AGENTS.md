...
- 当前 committed generation 发生 `WRITE_OUTCOME_UNKNOWN` 时仍须阻断该代执行、清理和重放；但只要后继 generation 已按完整配置、签名、账号/资源和 effect 合同准备完成，提交事务可锁定并核验精确的 `BLOCKED + WRITE_OUTCOME_UNKNOWN` 前代后原子切换路由。旧代及未知写 lease 永久作为不可删除的审计归档保留，不再阻断新代健康；普通 `BLOCKED`、缺少未知写 lease 或仍为当前代的未知写一律继续 fail closed，异步旧代 finalizer 不得把新 committed 路由改回阻断态。
