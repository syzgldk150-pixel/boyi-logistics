...
- `sync_arrival_stats` 的当天主单范围固定为“目标日 arrive-list ∪ 目标日实际到件扫描主单”；arrive-list 有但未扫描且历史未到齐的单号以到货 0 保留，目标日前有效成功快照已确认到齐、且目标日未再次扫描的 arrive-list 重复主单必须过滤，目标日实际重扫主单始终保留。累计扫描索引只计算当天范围内主单的累计到货件数，并以开单件数为上限，超量扫描必须封顶并返回 `quantity_adjustments` 审计计数。每次成功统计后必须用本次内存中的 A:S 统计结果刷新目标 Sheet 与 MySQL 未齐快照，不依赖人工发送“分批”；全部到齐时清空目标旧行并保留表头。`phase7.pending_arrivals_sheet` 是可选输出，资源未配置或写入失败时只返回 skipped，不得让主统计失败。自动刷新不得触发融辉差错或问题件上报。
...
- `arrive-list` 只保存预计到货；`sync_arrival_stats` 所有输出完整成功后才激活实际到货快照。同日最后一份成功快照为该日权威版本；每票目标日前最近一份有效成功快照中 `应到 > 0` 且 `已到 >= 应到`，才构成跨日重复过滤的历史到齐证据，历史已到为 0 或仍未齐必须保留。
