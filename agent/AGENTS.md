...
- 每个持久化定时任务默认 `REQUIRE_EACH_RUN`。迁移 018 前的任务级 `EXACT_SCHEDULE_EXEMPT` 只对 Scheduler 生效；手工、Console、飞书、Webhook 始终逐次审批。018 将当前发行的 57 条计划改写为 `automation.<automation_id>.run`，配置保存会安全退休旧任务级 EXACT；release hold 下的一次性项目策略 bootstrap 只有在旧 018 备份、原 EXACT grant、配置退休事件、typed committed generation 和当前行全部闭合时，才把整个项目转为 `LEGACY_SCHEDULE_ONLY`。首次分布固定为 16 个项目中的 10 个 LEGACY、6 个 `REQUIRE_EACH_RUN`，并绑定 55 条已启用旧任务证据；任一代际、配置或合同漂移都会在运行时降回逐次审批。
- 生产已应用的 `014_control_plane_task_cutover.sql` 按线上校验和保持字节不可变；后续合同修正只能通过 `016`/`017`/`018` 前向迁移。018 的 71 条历史身份由 57 条当前发行 typed schedule 与 14 条延期 R7 组成；R7 只验证原状态、绝不注册 Job。首次 post-018 发布必须由 marker 的原始 release SHA、16 个隐私安全 source snapshot、项目策略事件和 generation snapshot 重算通过；后续发布允许管理员合法改 schedule/策略，但 REQUIRE 必须绑定当前代际，FULL_AUTO/LEGACY 的旧绑定只能作为安全 stale 状态并按逐次审批解释。
