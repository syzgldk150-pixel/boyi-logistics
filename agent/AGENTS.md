...
- 排查线上报错时先看 `/health.instance_id` 区分来源：本地 WSL 通常是开发机主机名，ECS 应为服务器实例；不要把本地 MySQL/TMS 状态误判为 ECS 状态。
...
- 交互为 dry-run 编号列表 → 数字/多选/区间/“全部”选择 → 回显选择 → “确认”正式执行；两个 pending 阶段均为 10 分钟，重新发送“分批”会丢弃旧选择并刷新列表。
