...
- 只有在用户明确要求显式目标、`-SkipRestart`、`-SkipHealthCheck` 等特殊参数时，才偏离 `auto`。
...
- Service v2 插件的唯一可执行前端例外是 Manifest `settings_ui.entry=settings/index.html` 指向的包内静态设置页；它只能在 Console 的无同源 sandbox iframe 中经会话绑定 `postMessage` 桥读取脱敏账号状态、飞书资源目录和自身配置，并原子保存不透明绑定引用。它不能访问父 DOM、凭据、Cookie、Token、任意 HTTP、外网、下载、弹窗或顶层导航。设置保存必须保留 Console 调度，调度保存必须保留插件配置。每个新 v2 Manifest 至少声明一个中文 Harness contribution；生产只投影 committed、已配置、已启用且稳定的 `read/compute` 能力，写能力没有工具专属 exact preview 时保持拒绝。
...
- 同一 `automation_id` 实行项目级 single-flight：Command 接受事务先锁定项目与配置，只把刚接受待领取的 `RECEIVED/CONTEXT_READY/PLANNED/VALIDATED`、有效 Run/generation 租约、真实 `RUNNING/VERIFYING` Step 和 `FAILED_RETRYABLE` 视为互斥；已停止且无租约的历史未知写或闭合写回执保留事项、Evidence 与审计但不再阻止全新 Command，也不由安全清理改写。无写入事实的孤儿、登录或数据暂停 Run 每次最多原子取消 100 条，剩余历史不拒绝新任务。同一幂等请求仍复用原 Command，并发不同请求只能接受一个。Scheduler 只在尚未接受且发生 `PERSISTENCE_UNAVAILABLE` 时以同一身份按 2/10/30 秒重试；事务提交成功后才允许入口发送“已开始”，进度按状态变化去重并只发送一次最终结果。WorkflowRunner 默认运行 4 个有界 Worker，浏览器工具最多 3 个并发，分别可由 `WORKFLOW_RUNNER_CONCURRENCY` 和 `WORKFLOW_BROWSER_CONCURRENCY` 覆盖；浏览器步骤按精确账号会话加锁，同账号以及未绑定账号的会话串行，不同账号可受控并行。只读/计算工具不使用写互斥锁，受保护写仍只按“账号 + 外部目标 + 资源 + 写类型”精确串行。锁身份不完整时显式阻断，禁止按工具名或 heavy 类别猜测锁。Run 对外投影排队、读取、处理、写入、核验与完成阶段，实际阶段计时只从取得通道后开始。工具子进程、取消和 generation lease 均以 Run + Step 精确绑定。
- Outbox 按 Runner/Audit、飞书审批、审批过期、财务失败告警和财务分析分消费者领取；领取/确认在短事务中完成，慢处理在事务外执行。财务分析必须提交独立的受管 Command/Run 并由 ToolExecutor 子进程执行，不得在 Outbox 或 Uvicorn 事件循环内同步等待，也不得继承父 Run 的审批。
...
- Service v2 ZIP 插件的 Manifest、无签名超级管理员安装、Host API、托管存储和双轨迁移合同见 `../docs/plugin-platform-v2.md`。v2 必须是受 Bubblewrap、固定 `prlimit` 与无网络命名空间共同约束的 Linux Python 3.10 隔离子进程，服务名固定为 `plugin.<plugin_id>.<service>@<major>`；Provider 操作必须闭合声明 `{name,effect}`，其中 effect 仅限 `read/compute/internal_write/external_write/destructive`。Host capability 的 effect 与 input/output Schema、handler、账号/资源要求和运行限制由 `automation_plugins/host_capability_registry.py` 按精确 `(api_version, capability, action)` 权威定义；Manifest 只能声明 `capabilities[*].operations` 的 action 字符串，不能自行声明 Host effect。风险、锁、Evidence、重试、Harness 与 Broker 投影从 effect 机械派生，禁止名称猜测或复用 lifecycle `effect_kind`。跨插件只经声明依赖和服务注册表的 `service.invoke`；静态 grant 只开放受保护的动态 effect 分发，真正的 effect ceiling 来自当前 contribution 的精确治理，调用前还必须解析并核对 Provider 的精确 effect。读/计算不产生写标记，写操作必须先留下宿主写尝试并以 Python-only Host 调用观测、独立回读 Evidence 和 ResultVerifier 闭环。Registry output Schema 只约束业务 `data`；Host 调用引用只能放在独立 Broker 信封与私有 observation，不得在 Schema 校验后注入 `data`。包不得携带凭据、自定义前端或 SQL/DDL。`browser.session` 只开放逐项受审 action，未注册的 browser/http/file/event 能力必须返回 `CAPABILITY_UNAVAILABLE`，不得旁路直连或旧 whole-tool。托管 collection 支持 CAS、普通索引完整等值查询和事务内 unique 约束；禁止范围、扫描、按 unique 查询与跨项目读取。Provider 停用、卸载或依赖失效必须先持久化关闭消费者 physical Scheduler，再撤销进程内服务/贡献路由；恢复时仅在 stable committed generation、desired schedule 与 migration owner 同时闭合后重新启用。卸载默认把文档标记为 `RETAINED`，永久清除必须是项目卸载后的独立超级管理员操作。运行时贡献投影、生成仓储调度门禁和项目 contribution 路由分别隔离在 `automation_plugins/service_v2_projection.py`、`../shared/automation_plugin_generation_runtime_repository.py` 与 `orchestration/automation_project_service_v2.py`。
- `automation_plugins/service_v2_projection.py` 的受管热投影承载 Service v2 的 Console、Scheduler、Harness、Feishu、Webhook 与 Event contribution。generation CAS 必须同事务写入 `PENDING_PROJECTION` token 和旧项目/策略/任务/审批策略前镜像；Provider reference、`ManagedContributionRegistry` 与 strict Scheduler reload 共用投影锁，完整刷新成功后才切 exact active generation 并 ACK `ACTIVE`。刷新或 ACK 失败必须以 token/base generation 条件化 reverse CAS，精确恢复旧 durable route 与旧进程投影后才允许重试；目标新代存在任何 lease 历史、未知写或任何并发漂移时不得反向恢复，而要标记 `BLOCKED`、关闭持久 Scheduler gate、撤销全部项目进程路由并 tombstone 动态 Job。权威空 generation 同样必须携带精确 expected registration 空集完成刷新和 active-map 清除；旧代即使仍有 lease 也只进入 DRAINING，不再占用或接收飞书/Webhook/Event 路由。重启恢复对 `COMMITTED/PREPARED/PENDING_PROJECTION` 要求完整 contribution journal；`TARGET/PREPARING/WAITING` 只接受可逐项验证的持久子集，再从 snapshot 原子恢复整代 PREPARED reservation；`DRAINING/DISPOSING/BLOCKED` 只恢复非路由诊断，`ROLLED_BACK` 不恢复 contribution。启动时先构造但不启动 Scheduler、绑定 strict/emergency 投影器并恢复 durable phase，ACK 完成后才启动；`PENDING_PROJECTION/BLOCKED` 不能领取新 lease。停用和卸载按相同顺序先刷新物理 Scheduler、再整代 withdraw；纯 service v2 没有已启用 Console/Scheduler/Harness/Feishu/Webhook/Event contribution 时不创建伪注册 marker。动态飞书整代 prepare 必须使用运行时同一 parser 拒绝宿主登录、任务取消、扫描确认、审批绑定与固定 Action v1 文本，任何同代或跨项目冲突均不得留下部分 reservation。动态 Webhook 以全局 exact `POST + route` 作为大小写敏感的路由身份，整代 prepare/切换/撤销/恢复均保持原子，冲突不得留下部分 reservation；Dispatcher 只接收 verified method、route 和稳定 `source_event_id`，Policy 在创建 Command 前和同一 UOW 内再次核对 exact Registry identity，项目、service、operation、业务参数、账号与资源只从 Registry 和签名项目合同派生。Non-durable Event 以全局大小写敏感的 exact event name 作为身份并服从同一整代原子语义；Dispatcher 只接收 verified event name 与稳定 `source_event_id`，调用面零 payload/业务参数，Policy 同样在 Command 前和接受 UOW 内重查 Registry，全部目标与参数来自 Registry 和签名项目合同。`managed_event_dispatcher READY` 只代表 `durable=false` 的无外部总线离线 best-effort backend，Command 接受后才持久化，接受前可能丢失；`durable=true` 仍为 `CAPABILITY_UNAVAILABLE`，不得降级。Harness 从 Registry 私有 immutable active snapshot 读取工具，每次调用前重解 exact active generation；注册必须绑定真实签名 `runtime_permissions`，并只接受 `read/compute + harness_allowed=true + broker_effect=read` 且无网络、浏览器、Office、文件或 Broker operation 的包。`agent/harness/` 不得加载环境、数据库、文件、网络、TMS 或飞书；`harness_application.py` 只绑定签名 MySQL 管理员、内存 Session 和可信项目调用 adapter，`harness_api.py` 只承载闭合内部 HTTP 投影。真实 LLM、固定业务读网关、生产 sandbox 与持久 Session 均为 `PRODUCTION_GATED`。Catalog 只输出闭合 `active_contributions` 与 `ACTIVE/STALE/INACTIVE`，Service v2 的 Console/Scheduler/Harness/Feishu/Webhook/Event 调用只接受 exact active `COMMITTED/READY` 记录；Console 只安全展示 Feishu/Webhook/Event active kind，三者不进入浏览器手工入口。`managed_webhook_router READY` 仅表示无网络 Dispatcher backend；真实公网 namespace/验签/反代/流量，以及真实 event source、payload/version、Outbox fan-out、ACK/retry/dead-letter/replay、跨进程全局仲裁、数据库迁移、部署及生产故障注入均为 `PRODUCTION_GATED`。`shared/runtime_events.py`、事务 Outbox、Host `event.publish`、`ACTION_V1`、Webhook 与 Feishu 路径保持不变。
- TASK-EXT-010 只注册 `waybill_entry.actions` 与 `waybill_entry.validators` 两个 exact Module Slot。Manifest `module_slots` 项字段精确为 `id/slot/title/service/operation/default_enabled`，目标 Provider effect 只允许 `read/compute`；`ManagedContributionRegistry` 只公开 generation-bound `{slot,handle,title}`，Policy 在创建 Command 前和同一接受 UOW 内重解 handle 并核对 exact committed/READY identity。签名 Console principal 的 GET `/internal/v1/automation-projects/module-slots/waybill-entry` 只返回该安全投影，单 handle POST `/{slot}/{handle}/invoke` 与权威 active validator POST `/validators/invoke-active` 都只接受 `{request_id,waybill}`；waybill 的 21 个字符串字段只由 `shared.waybill_entry_extensions` 定义，调用方不得提交项目、generation、service、operation、effect、账号、资源、Actor、角色、handle 集合或任意 args。active-set Host 对 validators-only snapshot 前后精确比对并逐个等待单一完成结果，只返回闭合 `{kind:"validator_set",validation:{valid,issues}}`；超时、失败、结果或代际集合漂移关闭失败，稳定空集合才放行。停用/卸载随整代撤销 handle。不得连接真实 TMS/飞书/生产数据库或执行外部写；无数据库迁移，生产写与部署继续 `PRODUCTION_GATED`。
- TASK-EXT-011 以 `automation_plugins/connector_registry.py` 承载宿主代码拥有的只读 Connector 合同，并与 ZIP `ServiceRegistry` 严格分离；ZIP `provides` 与 contribution target 仍只接受 `plugin.*`。Manifest 只有精确 `{service,account_role}` 且对应账号角色 `required=true` 时才能依赖 `connector.*`；`core_adapter.py` 从项目当前精确绑定生成私有 `ConnectorBindingRef`，`capability_proxy_v2.py` 只向适配器传递该引用并向插件返回闭合脱敏结果。`fixture_connectors.py` 的 `connector.fixture.tracking@1/query` 仅由离线测试显式注入，生产组合默认空 Registry；Catalog 单独输出只读 `connectors` 投影且无生命周期动作。真实 TMS、飞书、数据库与写 Connector 均为 `PRODUCTION_GATED`。
- `service_v2_process_runtime.py` 是 AI 助手与 Webhook/Event 进程 backend 的 composition boundary。AI 助手直接复用当前激活的统一 `LLMClient`，状态为 `READY/ONLINE_READ_ONLY`，并开放六类代码拥有的只读业务网关；自然中文对话不再依赖 Bubblewrap，也不再接受固定命令格式或离线模拟回退。动态扩展工具仍必须来自 exact active snapshot，并满足中文名称、`read/compute + harness_allowed=true + broker_effect=read` 及关闭的运行权限。Session 仅进程内保存。`ServiceV2ManagedIngress` 只公开已验证 Webhook/Event identity 的两个进程内调用，持久注册与当前绑定同时有效才是 `READY`，未绑定返回 `PROJECT_RUNTIME_PROJECTION_STALE`，不新增 HTTP 路由。tracking fixture 仍只能由显式 `connector-test` 命令加载；公网 Webhook、外部 Event 源、真实 Connector 写能力和持久 Session 仍为 `PRODUCTION_GATED`。
- 插件协调按项目隔离：项目身份重复或缺失继续整体失败；单个项目的候选包、绑定或 generation 校验失败只把该项目候选标为 `UNAVAILABLE` 并记录脱敏健康原因，稳定 committed last-known-good 路由继续服务，其他插件和核心工具不得被连带阻断。只有候选完整校验成功后才原子切换 generation。
...
- Console 的“核验并恢复”仅用于闭合历史未知 receipt，不是重新执行入口；浏览器只提交请求 UUID，Agent 只依据持久 receipt 与权威回读判定 `WRITE_VERIFIED`、`NOT_APPLIED` 或 `UNKNOWN`，不得接受浏览器提交 generation、lease 或 evidence，也不得重放原 Run。`sync_scan_codes` 的新触发可在接受前有界枚举当前及已归档 generation 的旧未知 lease，按旧正式 Command 绑定的已验签预览精确恢复单号/下一站；缺少新版紧凑预览上下文的历史 Command 必须先验证同一 Run 的完整 Plan hash，再优先从 Plan Step 的服务端绑定、否则从 Plan impact 恢复并逐字段匹配原完成 Step，不得依赖历史内层 fingerprint 的重新序列化。回读同时覆盖旧版 naive UTC 与 naive 上海时间解释查询融辉发件扫描账本；所有精确身份在全部时间窗都为零时才落 `NOT_APPLIED` 并接受全新 Command，部分、重复、登录失败、结构异常或非零不闭合结果继续保持 `UNKNOWN`。其他普通重新执行直接提交新 Command，不得把未知写当作已取消。仅签名 MySQL `super_admin` 可通过 Console 的显式取消动作、填写非空原因后把已经暂停的未知写 Run 与 Work Item 终结为 `CANCELLED_UNKNOWN_WRITE_BY_SUPER_ADMIN`；该动作只解除项目互斥，不改写旧 receipt、lease 或 Evidence，也不把原 Run 伪装为成功或重放。
...
- Run/Work Item 状态转换必须走模型允许表和版本 CAS。登录恢复、补充信息恢复原 Run；`PARTIAL` 或终态失败创建关联新 Run。第三方/财务写的未知结果必须 `BLOCKED_DATA/WRITE_OUTCOME_UNKNOWN`，除非存在精确读后 reconciliation。到货统计最后一步的同一“未到问题件”Sheet 快照替换若存在多条历史未知 receipt，只能在实时主表与内部投影严格一致、目标受管范围严格为单表头基线时，按各 receipt 身份逐条落为 `NOT_APPLIED`、终结旧 Run、关闭旧事项，并用全新 Command 重试当前触发；参数不同的旧 receipt 不得按当前内容伪报 `APPLIED`，其他外部写和财务写也不得复用该恢复。同一 Scheduler task 与项目后续成功时，只能在完成事务内取消最新 Run 为 `FAILED_TERMINAL` 的旧 `OPEN` 事项并保留完整审计；候选必须按 Run→Command→Work Item 锁序重验，未知写、阻塞、非终态和已产生后续 retry 的事项均不得收口。迁移 `037` 是 single-flight 上线后的有界历史修复：只给 2026-09-04 上海零点前、`arrive_list` 项目、Scheduler 来源且已停止执行的三类阻塞 Run 写入系统取消请求，仍可执行或仍持有租约的 Run 必须使迁移失败；迁移 `038` 只收口仍带 `migration-037` 系统取消标记、无活动租约且 Run/Work Item 阻塞状态一致的孤立记录，并在同一事务写入标准状态事件与 Audit Outbox。两者都不得重放旧写或扩大到其他项目。
...
- 新 Command 固定生成 Schema v2 计划：哈希只覆盖实际工具版本、当前项目 generation、所选账号/资源、规范化参数、实际影响、Evidence 与写后条件；无关插件、无关账号、UI 或目录变化不得使审批失效。目标、账号、金额、工具版本、项目 generation、证据合同或写后条件变化仍必须重新审批。已在等待审批的 Schema v1 Run 继续按 v1 重算至终态，不批量改写历史记录。
- Scheduler 对持久任务逐行校验并隔离；非法行只进入该行健康告警，其他有效任务继续注册。只有重复启用任务 ID 等真实全局身份冲突才整体失败；刷新失败保留旧 Job 集。
...
- `shared/business_modules.py` 定义 15 个固定模块身份；固定模块的新 Command 不再查询或锁定旧生命周期行，其可用性只受代码路由、调用方身份权限和各业务自身前置条件约束。`migrations/027_business_module_lifecycle.sql`、历史表和 Lite 审计继续保留且不得改写；历史迁移固定保留 14 行 seed，不得为 Harness 改写；`/internal/v1/admin/modules` 只保留签名管理员读取目录、详情和审计，生命周期 POST 已移除。迁移合同仍在 seed 前精确校验结构且只补齐历史缺行。
...
- 自提问题件的逻辑来源必须保持为已审阅的“大祥到货数据”工作表 `UeBd3I`。该表由 `FILTER` 动态数组生成，而飞书开放接口不返回其溢出单元格；因此专用资源还必须固定记录公式源工作表 `8fc516`，生产适配器以 `FormattedValue` 读取该源后执行与大祥表相同的站点筛选，不得从项目的分批资源绑定动态推断或回退。分批/未到问题件继续读取其独立资源。启动同步必须恢复这组持久化定位；发布启动只把历史遗留的 `phase7.arrive_secondary_sheet` 项目绑定迁移到 `phase7.self_pickup_source_sheet`，已正确或管理员自定义的资源绑定不得覆盖。
