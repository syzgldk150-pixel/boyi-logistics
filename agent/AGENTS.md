...
- WorkflowRunner 默认运行 2 个有界 Worker，浏览器工具额外限制为单并发，分别可由 `WORKFLOW_RUNNER_CONCURRENCY` 和 `WORKFLOW_BROWSER_CONCURRENCY` 覆盖；只读/计算工具不使用执行互斥锁，受保护写只按“账号 + 外部目标 + 资源 + 写类型”精确串行。锁身份不完整时显式阻断，禁止退化成工具名、heavy 类别或全局默认锁。工具子进程、取消和 generation lease 均以 Run + Step 精确绑定。
