...
- `automation_plugins/service_v2_projection.py` 的受管热投影承载 Service v2 的 Console、Scheduler 与 Harness contribution。generation CAS 必须同事务写入 `PENDING_PROJECTION` token 和旧项目/策略/任务/审批策略前镜像；Provider reference、`ManagedContributionRegistry` 与 strict Scheduler reload 共用投影锁，完整刷新成功后才切 exact active generation 并 ACK `ACTIVE`。刷新或 ACK 失败必须以 token/base generation 条件化 reverse CAS，精确恢复旧 durable route 与旧进程投影后才允许重试；目标新代存在任何 lease 历史、未知写或任何并发漂移时不得反向恢复，而要标记 `BLOCKED`、关闭持久 Scheduler gate、撤销全部项目进程路由并 tombstone 动态 Job。启动时先构造但不启动 Scheduler、绑定 strict/emergency 投影器并恢复 durable phase，ACK 完成后才启动；`PENDING_PROJECTION/BLOCKED` 不能领取新 lease。停用和卸载按相同顺序先刷新物理 Scheduler、再整代 withdraw；纯 service v2 没有已启用 Console/Scheduler/Harness contribution 时不创建伪注册 marker。Harness 从 Registry 私有 immutable active snapshot 读取工具，每次调用前重解 exact active generation；注册必须绑定真实签名 `runtime_permissions`，并只接受 `read/compute + harness_allowed=true + broker_effect=read` 且无网络、浏览器、Office、文件或 Broker operation 的包。`agent/harness/` 不得加载环境、数据库、文件、网络、TMS 或飞书；`harness_application.py` 只绑定签名 MySQL 管理员、内存 Session 和可信项目调用 adapter，`harness_api.py` 只承载闭合内部 HTTP 投影。真实 LLM、固定业务读网关、生产 sandbox 与持久 Session 均为 `PRODUCTION_GATED`。Catalog 只输出闭合 `active_contributions` 与 `ACTIVE/STALE/INACTIVE`，Service v2 的 Console/Scheduler/Harness 调用只接受 exact active `COMMITTED/READY` 记录；`ACTION_V1` 路径保持不变，动态飞书/Webhook/Event dispatcher 仍未开放。
