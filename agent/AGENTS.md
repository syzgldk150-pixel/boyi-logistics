...
迁移 `051_separate_boyi_waybills.sql` 建立独立 `boyi_waybills` 表并事务迁移历史手工单，保存完整字段及 ID；Console 的博益开单仅写该表。
...
- 普通 reader 与插件执行适配器为当前工具、target 和必要 action 创建短期能力；工具子进程剥离内部 API Token、Console 签名密钥、会话/Webhook/验证 Token，只能用精确能力访问 `/tms/*`。业务入口不能借宽泛查询能力访问写端点。
...
- 飞书固定关键词、手动和定时使用受信项目 Invocation；客服、回单、物流和寄件查询使用已注册直接业务接口。兼容 `/tms/*` 仅接受精确短期执行能力，写动作必须提供稳定请求身份并经过对应业务校验，不提交旧 Command，也不绕过宿主能力访问第三方。
...
- 登录/验证码仍走账号管理接口；authenticated 状态只更新会话可用性，不恢复 BLOCKED_LOGIN 历史 Run、不重放原工具。新的业务请求由用户或正常定时重新发起。
...
- 现有生产业务的 Service V2 包及 Connector 已实现，业务算法在独立 ZIP 内执行，宿主只提供已审核接口。迁移按显式映射复制当前账号、资源、业务参数、启停和定时；TESTING/READY 时仅超级管理员可从 Console 验证目标，其他入口仍属于原实例；CUTOVER/COMPLETED 才接管，ROLLED_BACK 恢复。READY 必须有目标当前代的新鲜成功 Direct Invocation 与实际业务核验；切换等待已接受调用及写回读排空，历史失败/取消不形成锁。这些状态只解释历史迁移；当前发行要求权威 COMPLETED 归属及 Service-V2-only 退役索引，不恢复旧 V1 包。隔离测试不能替代实际业务验收。
...
- 新增插件使用 `scripts/service_v2_plugin.py init`，现行业务维护从 `service_v2_plugins/README.md` 定位，按 `scripts/plugin_maintenance.py` 局部测试、打包并通过后台安装或升级。`examples/automation_plugin/` 仅保留旧 Action V1 离线样例，不作为新增生产插件模板。
- 当前生产发布使用 `scripts/build_first_party_plugin_release.py --service-v2-only` 为最终 Git SHA 生成退役索引；不签名、不复用、不装载 V1 ZIP。`scripts/verify_first_party_plugins.py` 在发布时核验该索引与真实 COMPLETED 迁移归属。V2 业务修改通过独立 ZIP 升级精确实例，核心更新按 `deploy/publish_to_ecs.md` 操作；旧 Action V1 与 Windows Worker 合同仅保留为历史或未来能力资料。
...
- `scripts/first_party_release_scope.py` 以 AST 读取当前代码 allowlist；发布只复制 `service_v2_plugins/` 的当前包、`_shared/` 和明确允许的根文件，暂存树再次核验精确集合。`legacy/` 和旧 `first_party_automation_plugins/` 不得进入发布；缺包或夹带旧源码直接失败，不回落到旧算法。
...
- 自动化插件管理面位于 `agent/automation_plugins/management_api.py`、`management.py`、`management_repository.py` 与 `binding_resolver.py`：浏览器 DTO 必须闭合且身份只取签名 Console principal，生命周期/配置写只允许 `super_admin`；账号、资源和命名设备只精确匹配，不用默认或首项兜底。经安装器验证的原始 ZIP 必须以 `0600` 常规文件复制进不可变 installed 版本目录，插件运行器仅按 plugin/version/digest 安全读取；升级必须以 request UUID 在同一 UoW 注册目标版本、推进 desired generation、撤销旧授权，旧 committed generation 保持执行至新代原子切换，严禁覆盖当前版本或靠重启清场。Console 启用/停用也必须把 request UUID、目标状态、CAS 前后版本和管理员身份写入同一事务的 `PLUGIN_STATE_CHANGED` 事件；相同请求响应丢失时只读回精确结果，不得重复推进状态。
- Service-V2-only 启动验证退役索引与已完成迁移，按持久 committed generation 恢复当前 V2 注册，不自动恢复、安装或升级 V1。V2 版本升级由后台对精确实例执行，保留账号、资源、配置和入口；未排空代次、历史结果与未知写证据不删除。交互选择、预览指纹、客服复核引用和财务启动标记等代码拥有字段必须由精确首方身份声明、进入合同哈希并由 Agent 规范化，Console 不得让用户编辑或因其 Schema 形状阻断整个配置表单。
...
- 当前调用合同覆盖实际插件版本、committed generation、账号/资源、参数、能力、Evidence 和写后条件；无关插件、账号或页面变化不能使本次调用绑定漂移。旧 Schema v1/v2 Plan Hash 只用于历史证据验证，不为新 Invocation 构建空 Plan。
...
- 历史迁移说明（不代表当前执行入口）：迁移 `019` 保留生产已执行的 generation lease / Run 绑定原始字节；迁移 `020` 将所有现有 `REQUIRE_EACH_RUN` / `LEGACY_SCHEDULE_ONLY` 项目一次性改为 `PROJECT_FULL_AUTO` 并记录不可变审计。迁移 `021` 只恢复当前策略仍为 `PROJECT_FULL_AUTO` 的 typed `WAITING_APPROVAL` Run。迁移 `022` 只在最新不可变事件严格闭合为旧凭据安全降权或七键插件 `PLUGIN_VERSION_CHANGED` 降权时恢复持久完全自动；迁移 `024` 仅对原始六键 `PLUGIN_UPGRADE_STAGED` canonical 元数据与对应策略事件均严格闭合的旧插件降权执行同类修复，二者互不放宽并唤醒对应 typed Run。插件事件本身不能授予完全自动，较新的管理员事件永远优先。迁移 `023` 将历史重复 `ACTIVE` 故障安全退回 `QUEUED`、重置可证明的原始 Outbox 以重新通知，再以数据库唯一键保证每个绑定最多一条 `ACTIVE`；恢复 DML 必须显式事务化。当前项目权限由 committed 身份与持久管理员配置验证，配置/账号/依赖不闭合即明确拒绝；旧逐次审批行不能创建新的待领取任务。
...
- enabled_entrypoints 仍是签名入口清单的任意子集，允许空集；关闭入口必须由后端拒绝，关闭 Scheduler 保留时间配置但不注册 Job。飞书 actor 只从真实事件与当前有效 Console 超级管理员绑定产生，绑定码和管理员校验保留；旧 1/2 审批通知和 TTL 只用于历史记录，不再创建新的逐次审批等待。
...
- 统一回单管理：Console `/receipts/sync` 经普通 receipts-query 接口读取并保存本地结果，审核经 receipts-audit 直接业务动作并独立读后核验；两者不创建 Command/Run/Step，不要求事项中心审批。管理员签名、精确账号和稳定请求 UUID 保留；缺字段、多候选、登录失效或写后状态不一致明确失败。活动原页前缀仍为 410，本地照片、证据和结果页面保留。
...
- 交互通过互斥检查后先回复正在生成，再从该候选 Invocation 生成 dry-run 编号列表和选择快照。飞书 pending 只保存 `preview_invocation_id`、原发起人、候选/用户选择和到期时间，不保存账号或客户端指纹；确认与取消都必须精确匹配原发起人。确认时服务端重新加载同一已完成候选 Invocation，恢复受信任的 `preview_fingerprint` 与正式参数。原发起人只能在 15 分钟有效期内确认；自提默认选择全部候选，分批保留序号、多选和区间选择。Console 也只允许从已完成且验签的候选 Invocation 勾选子集。Scheduler、LLM 和旧同名工具均不能执行正式上传。
...
- 正式模式的 `selected_bill_codes` 与 `preview_fingerprint` 必须由 Agent 从 `preview_invocation_id` 指向的已验签持久化候选 Invocation 恢复，宿主确认适配器绑定一至九十个规范、唯一、有序运单号及指纹；飞书和浏览器都不能上传账号或指纹，旧 `split_pending_problem_upload` 直接工具仍固定 `IMPACT_PREVIEW_REQUIRED/BLOCKED_DATA`，不得绕过项目入口。
...
- 每日 00:10 同步前一完整业务日并回扫 7 天；逐笔汇总、平台汇总与 signed-net 必须一致，任何不一致都使本次调用失败，退出码 0 不能代替业务验证。
