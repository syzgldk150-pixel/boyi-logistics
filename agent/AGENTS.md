...
- R13 仅提供账号当前单号、R13 应签时间和参考签收状态；R13 消失、显示已签或空结果不得关闭。只有 TMS 主单扫描类型精确为“签收”的事件可关闭，子单签收无效。
- 应签候选、到货、问题件、签收证据统一写入迁移 `009_daily_sign_ledger.sql` 创建的共享表，并由 `tools/daily_sign_rules.py` 计算；禁止在其他脚本复制规则。
