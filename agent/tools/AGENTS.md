...
- `daily_sign_ledger` 中 B 口径为 R13 原始应签时间，C 口径为本系统测算时间；R13 必须按原页“规划应签收时间”口径查询，未显式传入起止时间时至少覆盖原页默认的前 2 天至后 3 天。包装类型不在该 R13 页面字段中，继续从实际到货或 TMS 运单详情取得。无实际到货时 C 与到货件数为空。权威台账保留完整候选，飞书应签明细只发布未签且有效应签时间不晚于当前业务日的记录：C 有值时以 C 为准，否则以 B 为准；普通电子表格必须先精确校验九列表头，再写新数据，成功后才清理尾部旧行。
