...
> 2026-08-12: 飞书脚本执行回执统一精简为执行结论和实际处理数量。全部成功显示“已完成”，部分成功显示“部分完成”，全部失败显示“未完成”；只在失败时追加失败项，不再展示来源表、内部飞书/数据库表状态、归档状态、截图数和耗时。自提问题件 dry-run 只保留候选数量及确认/取消提示；分批 dry-run 因支持序号选择，仍保留编号列表。
