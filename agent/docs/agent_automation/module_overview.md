...
> 2026-08-10: 融辉地址报价改为使用当前运单录入页的真实存储过程 `P_CALC_CLIENT_PRICE_BILL_SH_ZB`，并从地址解析后的 MiniUI 表单透传 `BL_INSURESTATUS` 计价状态；该字段缺失时显式失败，不再用不完整 payload 得出缺少末端派送费、直送服务费的低报价。折扣合计同步纳入页面现有的 `REC_PERIOD_FEE_DIS`。
