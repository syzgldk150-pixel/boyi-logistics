...
| 实时消息监控大盘（韵达 / 融辉 TMS / 今日应签未签） | `console/app.py` `console/templates/portal.html` `agent/tms_runtime/monitoring.py` `agent/tms_runtime/routes.py` | 首页通过 `/monitoring/summary` 和 `/monitoring/stream` 代理 Agent `/admin/monitoring/snapshot`；分类点击走 `/monitoring/detail-link` 获取原系统嵌入链接；“今日应签未签”卡片走 `/monitoring/daily-sign` 代理 `/admin/monitoring/daily-sign`，读取 `phase7.daily_sign_sheet` 飞书普通电子表格“应签明细”快照，按当日 `应签收时间` 计数，并按 `scheduled_tasks` 中启用的“每日应签”定时点返回刷新元数据；接口不返回明细、凭据、表格 token 或第三方请求体；只保存分类、数量、状态和非敏感跳转标识 |
...
| TMS/韵达 登录态、短信/图片验证码登录、`/tms/*` 兼容接口 | `console/templates/automation.html` `console/templates/automation_accounts.html` `console/app.py` `main.py` `agent/tms_runtime/account_manager.py` `agent/tms_runtime/session_broker.py` `agent/tms_runtime/routes.py` `agent/tms_runtime/dispatch.py` `agent/tms_runtime/scripts/` | `/automation-accounts` 账号管理只维护融辉/韵达/R7/R13 真实系统账号、凭据和登录态；脚本用哪个账号在 `/automations` 的 `account_roles` 角色绑定里选择，支持一个脚本多个账号角色（如自提部/大祥S站）；账号管理里的启用共享 session 账号会被监控，飞书监控强制校验后会回写 Agent `/admin/accounts` 列表缓存，Console 账号页批量轮询使用 `force=1&prefer_cached=1` 触发同口径刷新；图片验证码先 OCR，短信验证码由飞书 pending 接管；兼容业务入口统一挂在 Agent `/tms/*`；韵达报价入口是 `/tms/yunda_price`；韵达绿色登录态需通过主站、报表 `searchData`、`kyinms`、消息中心和 `kyproblem` 问题件页校验，非 JSON/空响应会短重试一次，明确登录页或重定向仍立即判过期 |
...
| Agent HTTP 接口、`/health`、`/chat`、`/run-tool`、`/admin/*` | `main.py` `agent/core.py` `agent/tool_executor.py` | 路由在 `main.py`，对话编排在 `core.py`，工具执行在 `tool_executor.py` |
