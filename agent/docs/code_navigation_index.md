...
| SERVICE_V2 ZIP 校验、Host Capability Registry、显式 effect、安装向导、贡献投影、托管存储与 v1→v2 迁移 | `agent/automation_plugins/manifest_v2.py` `agent/automation_plugins/package_v2.py` `agent/automation_plugins/host_capability_registry.py` `agent/automation_plugins/service_registry.py` `agent/automation_plugins/service_v2_contract.py` `agent/automation_plugins/capability_proxy_v2.py` `agent/automation_plugins/core_adapter.py` `agent/automation_plugins/lifecycle.py` `agent/automation_plugins/management.py` `agent/automation_plugins/management_api.py` `agent/automation_plugins/service_v2_projection.py` `agent/automation_plugins/production.py` `agent/automation_plugins/production_projection_identity.py` `agent/automation_plugins/execution.py` `agent/orchestration/planner.py` `agent/orchestration/plan_validator.py` `agent/orchestration/workflow_runner.py` `agent/orchestration/result_verifier.py` `agent/orchestration/automation_project_service_v2.py` `agent/orchestration/automation_project_policy_service.py` `agent/scheduler.py` `main.py` `../shared/automation_plugin_repository.py` `../shared/automation_plugin_v2_repository.py` `../shared/automation_plugin_generation_repository.py` `../shared/automation_plugin_generation_transition_repository.py` `../shared/automation_plugin_generation_runtime_repository.py` `../shared/automation_project_authorization.py` `service_v2_plugins/` `../console/services/extensions.py` `../console/services/automation_plugin_management.py` `../console/services/automation_projects.py` `../console/static/extensions.js` `migrations/033_plugin_service_v2_foundation.sql` `migrations/034_runtime_generation_activation_journal.sql` `../docs/plugin-platform-v2.md` | v2 是经签名 Console `super_admin` 上传的无签名 ZIP，严格以 `schema_version=2 + runtime_model=service_v2` 分流；解析失败不得回退 v1。Host Registry 是精确 API/capability/action 的 Schema、handler 与五态 effect 权威；Provider 操作使用 `{name,effect}`，Host `capabilities[*].operations` 仍是 action 字符串。逐 contribution target/governance 进入 generation、Plan、锁与 ResultVerifier；`service.invoke` 的静态 grant 只开放受保护的动态 effect 分发，真正的 effect ceiling 来自调用 contribution，实际 Provider effect 不得超过该 ceiling，读/计算保持零写标记。写成功必须闭合 committed generation、宿主写开始回执、Host 调用观测、独立 Evidence 和严格 postcondition proof。检查无副作用，最终安装重新验 ZIP；disabled 项目只有在配置、reconcile 与 committed generation 稳定后才启用。Bubblewrap、`prlimit`、无网络命名空间、托管 collection、固定 `PROJECT_FULL_AUTO` 安全投影和双轨迁移以根文档为权威；`docs/automation_plugin_platform.md` 只说明 ACTION_V1。Console/Scheduler 受管贡献先完整准备，再在同一投影锁内 strict reload APScheduler 并切换 exact Provider/Console generation，成功后才把 durable activation phase ACK 为 `ACTIVE`。刷新或 ACK 失败按 token/base generation reverse CAS，以进程 revision/身份拒绝覆盖更新投影并精确恢复旧项目、任务、审批策略与进程路由；目标新代存在任何 lease 历史、未知写或并发漂移会持久标记 `BLOCKED`、关闭 Scheduler gate、撤销项目路由并 tombstone 动态 Job。启动先创建但不启动 Scheduler、绑定 strict/emergency 投影器并按 durable phase reconcile/ACK，闭合后才 start。停用/卸载先刷新物理 Job 再整代 withdraw。Catalog 只投影白名单 active contribution 状态，Console 只开放当前 committed generation 的 `COMMITTED/READY` 记录；纯 service v2 不造 marker，ACTION_V1 不变，动态飞书/Webhook/Event dispatcher 与端到端 Harness 后续实现。Console 扩展中心只投影同一 Catalog，不创建第二套仓储或自定义前端。 |
