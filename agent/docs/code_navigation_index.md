...
| SERVICE_V2 ZIP 校验、Host API、安装向导、贡献投影、托管存储与 v1→v2 迁移 | `agent/automation_plugins/manifest_v2.py` `agent/automation_plugins/package_v2.py` `agent/automation_plugins/lifecycle.py` `agent/automation_plugins/management.py` `agent/automation_plugins/management_api.py` `agent/automation_plugins/service_v2_contract.py` `agent/automation_plugins/capability_proxy_v2.py` `agent/automation_plugins/service_v2_projection.py` `agent/automation_plugins/production.py` `agent/automation_plugins/execution.py` `agent/orchestration/automation_project_service_v2.py` `agent/orchestration/automation_project_policy_service.py` `../shared/automation_plugin_repository.py` `../shared/automation_plugin_v2_repository.py` `../shared/automation_plugin_generation_runtime_repository.py` `service_v2_plugins/` `../console/services/extensions.py` `../console/services/automation_plugin_management.py` `../console/static/extensions.js` `migrations/033_plugin_service_v2_foundation.sql` `../docs/plugin-platform-v2.md` | v2 是经签名 Console `super_admin` 上传的无签名 ZIP，严格以 `schema_version=2 + runtime_model=service_v2` 分流；解析失败不得回退 v1。检查无副作用，最终安装必须重新验 ZIP，并以包、闭合规范意图和操作者绑定根幂等；disabled 项目只有在配置、reconcile 与 committed generation 稳定后才启用，重放只续做缺失阶段。Bubblewrap、`prlimit`、无网络命名空间、Host API 能力代理、服务注册、托管 collection 和双轨迁移以根文档为权威；只有在 invocation 精确匹配 committed contract 且允许全自动后，Service v2 的安全投影与评估才固定 `PROJECT_FULL_AUTO`，遗留逐次审批值不得产生 `WAITING_APPROVAL`。`docs/automation_plugin_platform.md` 只说明 ACTION_V1。Console 扩展中心只投影该权威目录，不创建第二套仓储。 |
