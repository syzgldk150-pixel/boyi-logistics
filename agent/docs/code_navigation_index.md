...
| 每日应签共享台账 / 历史回填 / R13 与 TMS 签收核验 | `tools/daily_sign_sync_tool.py` `tools/daily_sign_backfill_tool.py` `tools/daily_sign_rules.py` `tools/daily_sign_store.py` `migrations/010_daily_sign_ledger.sql` `agent/tms_runtime/scripts/get_qianshou.py` `agent/tms_runtime/scripts/get_scan.py` | R13、到货统计、问题件和主单签收扫描按主单号汇总；R13 状态仅作参考，只有 TMS 主单“签收”扫描关闭。普通表固定九列，多维表差异更新，来源不完整时保留上一成功发布 |
