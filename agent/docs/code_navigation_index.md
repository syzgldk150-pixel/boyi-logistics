...
| 每任务定时审批策略、免审状态与动态发布静默窗口 | `../shared/scheduled_task_approval.py` `../shared/scheduled_task_approval_repository.py` `../shared/runtime_repositories.py` `agent/orchestration/scheduled_task_approval_service.py` `agent/orchestration/policy_engine.py` `main.py` `agent/scheduler.py` `../console/routes/automation.py` `../console/services/automation.py` `../console/templates/automation.html` `../console/static/automation_approval_policy.js` `scripts/run_migrations.py` `deploy/remote_release.sh` `migrations/015_scheduled_task_approval_policies.sql` | 默认 `REQUIRE_EACH_RUN`，仅签名 Console `super_admin` 可将符合 `registry.yaml approval.mode=schedule_allowlist` 资格的单项任务配置为 `EXACT_SCHEDULE_EXEMPT`。免审仅 Scheduler 生效；手工/Console/飞书/Webhook 仍逐次审批。服务端哈希绑定工具/版本、参数/账号、cron、enabled、治理字段、postconditions、动态规则和配置版本（不含名称），变更即 stale；发布器按有效外部写策略快照计算静默窗口。 |
