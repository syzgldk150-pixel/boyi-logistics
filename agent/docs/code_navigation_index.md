...
| 业务模块目录与生命周期 Lite | `../shared/business_modules.py` `../shared/business_module_repository.py` `migrations/027_business_module_lifecycle.sql` `scripts/run_migrations.py` `scripts/business_module_migration_contract.py` `agent/business_modules_api.py` `agent/orchestration/business_module_command_gate.py` `../console/services/business_modules.py` `docs/business_module_lifecycle.md` | 当前 14 个 Console 菜单身份是不可变代码目录；MySQL 仅保存安装状态和 Lite 审计事件。027 可在 DDL 中断后重跑：runner 显式加载迁移合同，在表创建后、seed 前精确校验结构，seed 只补齐缺行、不覆盖状态；审计只经唯一仓储/API 路径追加，不依赖 trigger。`/internal/v1/admin/modules` 读取要求已签名 Console 管理员，生命周期写严格要求 `super_admin`、UUID 请求键、非空理由和 CAS。14 个静态菜单始终可见；Agent 状态不可达时 GET 壳/只读入口保留而写请求 503，已知非 `ENABLED` 的可管理模块才阻断页面/API。Command Gateway 在同一 UoW 锁中拒绝其新的普通和项目化命令，核心模块不可停用，已受理 Run 可继续。 |
