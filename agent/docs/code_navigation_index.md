...
| TMS/韵达 登录态、短信/图片验证码登录、`/tms/*` 兼容接口 | `console/templates/automation.html` `console/templates/automation_accounts.html` `console/app.py` `main.py` `agent/tms_runtime/account_manager.py` `agent/tms_runtime/session_broker.py` `agent/tms_runtime/routes.py` `agent/tms_runtime/dispatch.py` `agent/tms_runtime/scripts/` | `/automation-accounts` 账号管理只维护融辉/韵达/R7/R13 真实系统账号、凭据和登录态；“立即登录”直接执行一次登录，自动登录只负责定时校验和掉线恢复；大祥报价 `price_default` 与默认融辉/韵达账号的兼容接口统一经过 AccountManager，但保留独立 `price` Cookie/会话目录，所有非默认 profile 只使用各自页面保存的凭据；脚本用哪个账号在 `/automations` 的 `account_roles` 角色绑定里选择，支持一个脚本多个账号角色（如自提部/大祥S站）；账号管理里的启用共享 session 账号会被监控，飞书监控强制校验后会回写 Agent `/admin/accounts` 列表缓存，Console 账号页批量轮询使用 `force=1&prefer_cached=1` 触发同口径刷新；图片验证码先 OCR，短信验证码由飞书 pending 接管；兼容业务入口统一挂在 Agent `/tms/*`；韵达报价入口是 `/tms/yunda_price`；韵达绿色登录态需通过主站、报表 `searchData`、`kyinms`、消息中心和 `kyproblem` 问题件页校验，非 JSON/空响应会短重试一次，明确登录页或重定向仍立即判过期 |
