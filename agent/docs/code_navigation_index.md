...
| 项目全自动策略、事项/飞书审批与 018/019 迁移 | `../shared/automation_project_policy_repository.py` `../shared/feishu_approval_repository.py` `agent/orchestration/automation_project_policy_service.py` `agent/orchestration/approval_service.py` `agent/orchestration/feishu_approval_service.py` `agent/orchestration/policy_engine.py` `agent/orchestration/workflow_runner.py` `main.py` `../console/services/auth.py` `migrations/018_automation_project_authorization.sql` `migrations/019_automation_full_auto_feishu_approvals.sql` | 019 将现有项目默认成持久化 `PROJECT_FULL_AUTO`，配置/插件代际变化不改写管理员意图；runtime 同步状态独立阻断旧配置运行。事项批准在事务内重新调度等待 Run 后唤醒 Runner。飞书用一次性绑定码继承实时超级管理员角色，并通过 Outbox 为每名管理员串行投递，精确回复 1/2 决定当前审批。 |
