...
| 需求类型 | 优先查看文件 | 说明 |
|------|------|------|
| Agent 控制平面、状态机、计划、审批、恢复与 Evidence | `agent/orchestration/models.py` `agent/orchestration/command_gateway.py` `agent/orchestration/context_builder.py` `agent/orchestration/planner.py` `agent/orchestration/plan_validator.py` `agent/orchestration/policy_engine.py` `agent/orchestration/approval_service.py` `agent/orchestration/workflow_runner.py` `agent/orchestration/result_verifier.py` `agent/orchestration/outbox_dispatcher.py` `../shared/orchestration_repository.py` `../shared/orchestration_repository_support.py` `../shared/orchestration_schema.py` `main.py` `docs/control_plane_v1.md` | 所有业务执行先持久化 Command/Work Item/Run；只有 WorkflowRunner 调 ToolExecutionPort；澄清只接受绑定原 command_id 的显式账号与 JSON 参数覆盖并重新校验；状态 CAS、租约恢复、计划过期、审批角色、Evidence 和事务 Outbox 都在此链路，不能在入口或工具中另建编排旁路 |
...
| 每日应签权威账本、精确退避核验与影子事项投影 | `migrations/010_daily_sign_ledger.sql` `migrations/013_daily_sign_verification_state.sql` `migrations/016_daily_sign_single_tms_account.sql` `tools/daily_sign_rules.py` `tools/daily_sign_store.py` `tools/daily_sign_sync_tool.py` `agent/tms_runtime/scripts/get_sign_records.py` `agent/orchestration/pilot_projection.py` `../tests/test_pilot_projection.py` | 独立 R13 来源账号加一个融辉 TMS 邵阳大祥站账号显式绑定；唯一 TMS 登录态统一用于问题件、主单签收、轨迹核验和地址补全；完整分页与 31 天长历史分片不变；R13 只作诊断，只有真实主单签收关闭；离开 R13 的候选按 1/3/7 天持久化退避复核；影子集合保存 hash/差异，未达连续三个完整业务日标准时不切首页 |
...
| 自动化插件签名安装、项目实例、代际热切换、低层 Broker 与首方动作提取 | `agent/automation_plugins/` `first_party_automation_plugins/` `plugin_core_adapters/` `agent/windows_worker/` `agent/windows_worker/tray_host.py` `agent/windows_worker/tray_ipc.py` `agent/windows_worker/installer.py` `windows_worker_host.py` `../shared/automation_plugin_repository.py` `../shared/automation_project_authorization.py` `migrations/018_automation_project_authorization.sql` `migrations/019_automation_generation_lease_run_binding.sql` `scripts/sign_automation_plugin.py` `scripts/build_first_party_plugin_release.py` `scripts/verify_first_party_plugins.py` `docs/automation_plugin_platform.md` `first_party_automation_plugins/MIGRATION_MATRIX.md` | 一个签名 ZIP 定义一个可复用动作；账号、参数、定时、审批和设备属于独立 automation instance。payload 只含 stdlib/SDK/声明依赖并通过精确 `(operation, action)` Broker 调闭合核心端口，不导入 Agent/Shared 业务模块、不接 whole-tool fallback。首方生产集合只包含迁移矩阵标为 `RUNNABLE` 且进入代码 allowlist 的 Linux/ECS 动作，构建器按该精确集合原子打包、预检；`BLOCKED` 动作不会因源码存在而进入 bootstrap、Catalog、Broker 或健康计数。新 generation 原子接绑定权威 Run 的新租约，旧 lease 排空后清理；遗留未绑定 lease 不得猜测恢复，末引用卸载才删除包和 venv。Windows Worker/Tray 与两个 R7 打卡动作本轮整体延后，不挂路由、不参与发布健康，也不阻断其他服务端插件；未来恢复仍须满足有界 IPC、交互会话重验、未知写清理阻断和闭合 adapter 等合同。 |
...
| 融辉财务工作台（韵达待启用） | `console/templates/finance.html` `console/static/finance.js` `console/static/finance.css` `console/finance_service.py` `console/services/monitoring_finance.py` `shared/finance/sources.py` `agent/tms_runtime/scripts/finance_live_capture.py` `agent/tms_runtime/scripts/ronghui_finance_adapter.py` `tools/finance_sync_service.py` `tools/sync_finance_bills_tool.py` `agent/finance_brain.py` `agent/scheduler.py` `docs/finance_module.md` | 当前生产只启用来源注册表中的融辉三个财务角色，韵达不调度、不展示、不计入告警；手工同步、回填和重试使用真实管理员会话向 `/internal/v1/commands` 提交并等待 `super_admin` 审批；每日 `00:10` 同步前一完整业务日并回扫 7 天；逐笔、平台汇总与 signed-net 必须一致；金额使用 Decimal / `DECIMAL(20,4)`，旧 Excel ETL 已从运行时删除 |
...
| 自动化页表单、插件实例配置、账号/资源选择与系统定时 | `../console/templates/automation.html` `../console/static/automation_approval_policy.js` `../console/services/automation.py` `../console/services/automation_projects.py` `../console/routes/automation.py` `agent/workflow_resource_store.py` `agent/scheduler.py` | Catalog 驱动项目卡；账号/资源按签名角色精确选择且无默认首项，资源 DTO 不暴露运行配置。插件不带实际定时，安装后由系统保存类型化 schedule 并由 Scheduler 生效。 |
| 每任务/项目定时审批策略、018 一次性授权迁移与动态发布静默窗口 | `../shared/scheduled_task_approval.py` `../shared/scheduled_task_approval_repository.py` `../shared/automation_project_policy_repository.py` `../shared/runtime_repositories.py` `agent/orchestration/scheduled_task_approval_service.py` `agent/orchestration/automation_project_policy_service.py` `agent/orchestration/policy_engine.py` `agent/orchestration/workflow_runner.py` `main.py` `agent/scheduler.py` `scripts/automation_project_release_manifest_preflight.py` `scripts/migration_018_authorization.py` `scripts/run_migrations.py` `deploy/remote_release.sh` `migrations/015_scheduled_task_approval_policies.sql` `migrations/016_daily_sign_single_tms_account.sql` `migrations/017_scheduled_task_contract_upgrade.sql` `migrations/018_automation_project_authorization.sql` | 默认逐次审批。018 配置保存先退休旧任务级 EXACT；release hold 下一次性项目 bootstrap 只有在 pre-image、原 grant、退休事件、typed committed generation 全部匹配时才建立项目级 LEGACY，并以 marker/source snapshot 绑定。首次 post-018 manifest 精确核验 71/68/16 与 10 LEGACY/6 REQUIRE；后续允许合法 schedule/策略变化，但 stale 授权只解释为逐次审批。Scheduler 对 14 条 deferred R7 只校验身份并零注册；runner 负责状态捕获、精确回滚、首次/后续只读门禁及受保护写静默。 |
...
| 自提到货问题件 | `first_party_automation_plugins/self_pickup_problem_upload/payload/action.py` `agent/automation_plugins/problem_handlers.py` `plugin_core_adapters/problem_actions.py` `agent/tms_runtime/scripts/ronghui_problem_upload.py` `tools/registry.yaml` | 签名包从精确绑定的飞书表筛选 `邵阳自提部` 以及 `邵阳大祥S站 + 派送方式=自提` 且到货件数严格等于货物件数的候选；正式执行必须提交当前预览指纹和显式运单集合，并在所有候选预检通过后逐票写入。账号与表定位只在 broker 侧解析；保存回执不构成成功，必须由独立登记问题件列表唯一匹配运单、外部 ID、类型、归属、原因指纹和登记时间。 |
...
| 需求类型 | 优先查看文件 | 说明 |
|---|---|---|
| 分批差错及问题件 | `first_party_automation_plugins/split_pending_problem_upload/payload/action.py` `agent/automation_plugins/problem_handlers.py` `plugin_core_adapters/problem_actions.py` `tools/phase7_mysql_store.py` `agent/tms_runtime/scripts/ronghui_split_complaint.py` `agent/tms_runtime/scripts/ronghui_problem_upload.py` `feishu/message_handler.py` | 签名包严格解析 19 列来源、整数件数和问题分类，使用当前预览指纹与显式运单集合，先完成全部投诉/问题件预检，再更新精确绑定的快照与目标表并逐票执行。投诉保存回执或重复提示都不算成功，必须由独立投诉列表唯一匹配完整计划指纹；问题件同样要求独立登记列表唯一匹配。目标表、MySQL 快照/结果与每日应签问题事件均在精确写后读回一致时才提交成功。 |
