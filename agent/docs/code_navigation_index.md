...
| 业务模块目录与生命周期 Lite | `../shared/business_modules.py` `../shared/business_module_repository.py` `migrations/027_business_module_lifecycle.sql` `agent/business_modules_api.py` `agent/orchestration/business_module_command_gate.py` `../console/services/business_modules.py` `docs/business_module_lifecycle.md` | 当前 14 个 Console 菜单身份是不可变代码目录；MySQL 仅保存安装状态和不可变审计事件。`/internal/v1/admin/modules` 读取要求已签名 Console 管理员，生命周期写严格要求 `super_admin`、UUID 请求键、非空理由和 CAS。可管理模块的菜单、页面和 API 会随状态关闭；Command Gateway 在同一 UoW 锁中拒绝其新的普通和项目化命令，核心模块不可停用，已受理 Run 可继续。 |
