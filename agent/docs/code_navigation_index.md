...
| 控制台菜单/权限注册、首页、模块页、导航文案、页面乱码、布局样式 | `../console/navigation.py` `../console/permission_registry.py` `../console/app.py` `../console/templates/base.html` `../console/templates/portal.html` `../console/static/style.css` `../shared/service_identity.py` | `navigation.py` 以不可变注册项声明稳定菜单身份并投影旧导航结构；`permission_registry.py` 为每个菜单登记一个查看权限，只复用签名身份允许的 `admin/super_admin` 角色，不替代路由认证和超级管理员写边界。模块状态不进入菜单或权限注册。页面结构和文案在模板，公共状态数据在 `app.py`，样式统一在 `style.css` |
