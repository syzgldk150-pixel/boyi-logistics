...
| 每任务定时审批策略、免审状态与动态发布静默窗口 | `../shared/scheduled_task_approval.py` `../shared/scheduled_task_approval_repository.py` `../shared/runtime_repositories.py` `agent/orchestration/scheduled_task_approval_service.py` `agent/orchestration/policy_engine.py` `agent/orchestration/workflow_runner.py` `main.py` `agent/scheduler.py` `../console/routes/automation.py` `../console/services/automation.py` `../console/templates/automation.html` `../console/static/automation_approval_policy.js` `scripts/run_migrations.py` `deploy/remote_release.sh` `migrations/015_scheduled_task_approval_policies.sql` `migrations/016_daily_sign_single_tms_account.sql` `migrations/017_scheduled_task_contract_upgrade.sql` | 默认 `REQUIRE_EACH_RUN`，仅签名 Console `super_admin` 可将符合 `registry.yaml approval.mode=schedule_allowlist` 资格的单项任务配置为 `EXACT_SCHEDULE_EXEMPT`。免审仅 Scheduler 生效；手工/Console/飞书/Webhook 仍逐次审批。服务端哈希绑定工具/版本、参数/账号、cron、enabled、治理字段、postconditions、动态规则和配置版本（不含名称），变更即 stale；恢复 RUNNING Run 会重新评估策略，未开始写在免审失效后回到审批，已开始写只 reconcile；runner 提供 `016`/`017` 状态捕获、完整行恢复、bootstrap 精确清理、首次/后续 manifest 健康门禁及运行中受保护写门禁。 |
...
| R7 到达/发车打卡（本轮插件发行延后） | `tools/r7_arrival_checkin_tool.py` `tools/r7_departure_checkin_tool.py` `agent/tms_runtime/scripts/auto_checkin_r7.py` `agent/tms_runtime/scripts/auto_departure_r7.py` `agent/direct_tool_router.py` `feishu/message_handler.py` `console/app.py` | 旧工具源码只作定位；`r7_arrival_checkin`、`r7_departure_checkin` 不进入当前签名工件、bootstrap、Catalog、Broker 或健康门禁，待权威页面证据与闭合 adapter 齐备后另行恢复 |
