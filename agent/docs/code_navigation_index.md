...
| 控制台菜单注册、首页、模块页、导航文案、页面乱码、布局样式 | `../console/navigation.py` `../console/app.py` `../console/templates/base.html` `../console/templates/portal.html` `../console/static/style.css` | `navigation.py` 以不可变注册项声明稳定菜单身份、路由、文案、图标和分区，并投影旧导航结构；权限和模块状态不进入菜单注册。页面结构和文案在模板，公共状态数据在 `app.py`，样式统一在 `style.css` |
