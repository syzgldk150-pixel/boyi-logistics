...
| 融辉财务工作台与 Agent 经营汇总（韵达待启用） | `console/templates/finance.html` `console/static/finance.js` `console/static/finance.css` `console/finance_service.py` `console/services/monitoring_finance.py` `shared/finance/sources.py` `shared/finance/repository.py` `agent/business_query.py` `agent/tms_runtime/scripts/finance_live_capture.py` `agent/tms_runtime/scripts/ronghui_finance_adapter.py` `tools/finance_sync_service.py` `tools/sync_finance_bills_tool.py` `tools/business_finance_query_tool.py` `agent/finance_brain.py` `agent/scheduler.py` `docs/finance_module.md` | 当前生产只启用来源注册表中的融辉三个财务角色，韵达不调度、不展示、不计入告警；`query_business_finance` 在单个只读一致性事务中从共享账本返回已验证期间汇总，要求逐账号逐日最新终态均为 success/no_data 且 validation=passed，并做原始 Decimal 对账；不接受 SQL、账号绑定或任意数据源，当前不向 LLM/飞书开放；自然语言入口属于 TASK-051 |
