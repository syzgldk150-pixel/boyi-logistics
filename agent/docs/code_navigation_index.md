...
| Phase 7 / 自动化同步链路 / 定时同步链路 | `tools/*sync_tool.py` `tools/phase7_sync_common.py` `tools/split_pending_snapshot.py` `agent/workflow_resource_store.py` `agent/task_templates.py` `agent/scheduler.py` `agent/orchestration/automation_run_supersession.py` `../shared/automation_run_lookup.py` | 同步逻辑在 `tools/`，入口统一提交 Command；项目互斥只认有效租约、真实运行/核验步骤和待自动重试 Run，已停止的历史未知写保留 Evidence 但不再阻止新 Command；Scheduler 只对尚未接受的瞬时持久化失败按 2/10/30 秒保留原身份重试。到货统计范围为目标日 arrive-list 与目标日扫描并集，过滤历史已到齐未重扫主单、保留历史未齐零到货主单并按开单件数封顶；`scan_window_days` 只允许 1 |
