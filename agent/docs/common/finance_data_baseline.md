...
在接受财务分析指令后，请严格按以下步骤执行，并展示可复核的方法、来源与校验结果：
...
> 使用建议：开始财务数据任务前，先读取本文件
> `agent/docs/common/finance_data_baseline.md`，并同时遵循
> [财务模块说明](../finance_module.md) 中的当前数据源、命令和验证口径。
