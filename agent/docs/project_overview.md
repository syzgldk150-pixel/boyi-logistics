...
- `财务对账`
  当前以已验收的融辉真实财务页面为唯一生产来源，负责逐笔账本、平台汇总、费用项目绑定、运单财务事实、BI 和同步审计；韵达财务适配器待真实页面验收后再启用。
- `车辆调度`
...
3. `融辉真实财务页面 -> 采集校验 -> 共享账本 -> 财务对账`（韵达财务待启用）
   逐笔交易与平台汇总经账号、网点、分页、金额和余额校验后写入版本化账本，再生成费用分类、运单事实与 BI 查询结果。
4. `OCR + 价格 + 车辆信息 -> 车辆调度`
...
- `/modules/pricing`
  价格获取模块说明页。
- `/modules/finance`
  融辉财务工作台，提供 BI 总览、交易明细、费用项目绑定和同步记录；韵达财务未纳入生产获取。
- `/dispatch`
...
- 目前可交互运行的是 `OCR 工作区`、`融辉财务工作台` 和 `车辆调度工作区`。
- 财务模块通过共享 MySQL 账本与 Agent `sync_finance_bills` 接通；共享来源注册表当前只启用融辉，韵达历史失败保留审计但不计入当前告警；项目不再保留 Excel/CSV 离线财务 ETL 或工作簿报表入口。
...
- `sync_arrive_list` 当前拉取 TMS「派件预报」作为到货基础清单；`sync_arrival_stats` 每次重新拉取目标日派件预报与目标日到件扫描，以两者主单并集生成当天统计范围。派件预报有但目标日未扫描、且历史未到齐（包括历史到货为 0）的单号保留为到货 0；每票目标日前最近一份有效成功快照已到齐且目标日未再次扫描的重复主单过滤；目标日实际重扫主单始终保留。
- 2026-05-18：`sync_arrival_stats` 会把 `20055750680002` 这类融辉纯数字子单归并到主单 `2005575068`，并在统计导出时过滤历史缓存中的子单行，避免旧误入库子单继续写入飞书。
- `sync_arrival_stats` 默认以扫描索引中的子单扫描数作为到货件数；不会用主单总件数补高到货数，避免把仍在上游分拨的子单误算为到货，但累计扫描数高于开单件数时会封顶到开单件数。`count_result.quantity_gaps` 是“扫描子单数低于主单总件数”的审计提示，`count_result.quantity_adjustments` 是“累计扫描数超过开单件数并已封顶”的审计计数。
