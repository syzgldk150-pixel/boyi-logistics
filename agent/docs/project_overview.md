...
- `sync_daily_should_sign` 使用 R13 独立 SSO，账号资源通过 `workflow_resources.phase7.r13_credentials` 维护，不复用顶部共享 TMS 登录态。R13 候选只按真实字段 `signTime`（实际签收时间）和 `signSiteName`（签收网点）排除已签收记录，不再把 `dispTime`（派件时间）当作签收依据；写入应签明细前还会通过 TMS `delivery_status` 批量复核，只接受明确“未签收”的运单，签收状态缺失或不可识别时停止本次覆盖写入。
