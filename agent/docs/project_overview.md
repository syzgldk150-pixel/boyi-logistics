...
- `财务对账`
  以融辉/韵达真实财务页面为唯一来源，负责逐笔账本、平台汇总、费用项目绑定、运单财务事实、BI 和同步审计。
- `车辆调度`
...
- 财务模块通过共享 MySQL 账本与 Agent `sync_finance_bills` 接通；项目不再保留 Excel/CSV 离线财务 ETL 或工作簿报表入口。
...
- `sync_daily_should_sign` 使用 R13 独立 SSO，账号资源通过 `workflow_resources.phase7.r13_credentials` 维护，不复用顶部共享 TMS 登录态。R13 候选只按真实字段 `signTime`（实际签收时间）和 `signSiteName`（签收网点）排除已签收记录，不再把 `dispTime`（派件时间）当作签收依据；写入应签明细前还会通过 TMS「快件跟踪」主单扫描接口复核，存在 `SCAN_TYPE=签收` 的记录会排除，扫描结果缺失时停止本次覆盖写入。
