...
- 2026-08-11：账号页“立即登录”与自动登录开关语义拆分：前者立即执行一次真实登录，后者只控制定时校验与掉线恢复；大祥报价和旧兼容登录接口统一经过 AccountManager，`price_default` 仍保留独立 `price` Cookie/会话目录，非默认账号 profile 不再继承部署级账号密码。
