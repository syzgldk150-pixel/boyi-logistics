...
Console 的 `/automations` 是包清单、权限摘要、已安装实例健康状态、安装/升级/启停/卸载、定时和运行的唯一日常入口；侧栏不再注册扩展中心。`/extensions` 与旧详情 GET 只重定向到自动化或对应插件筛选，旧生命周期 POST 返回 410。所有入口读取同一个 Catalog 并复用同一生命周期 API，不能新增第二套插件仓储或用固定业务模块伪装扩展；查看要求真实非 legacy MySQL `admin/super_admin`，生命周期写仍只允许 `super_admin`。

Service v2 新安装固定先调用只读 `inspect-upload`，该请求只接收同一个 ZIP、Console 计算的传输摘要和检查 UUID；返回值只含权限、角色、配置 Schema、贡献和建议定时，不暴露 Manifest、服务目标、路径或摘要。最终 `install-v2` 必须重新核验同一 ZIP，浏览器只提交实例名和权限确认；服务器创建默认停用、未配置的独立实例，不接收通用账号/资源/入口/定时表单。仓储幂等键绑定包摘要、规范化最小意图和签名管理员身份；响应丢失后只允许原样重放，安装审计不得重复。插件业务设置通过包内 `settings/index.html` 隔离页和闭合桥保存，Console 定时通过独立调度事务保存，两者各自使用配置版本 CAS 并保留另一部分数据。启用仍要求匹配的 committed generation 稳定；未就绪只返回可重试的 `PREPARING`，不得要求重启服务或直接低层启用。
