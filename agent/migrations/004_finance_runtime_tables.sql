...
CREATE TABLE IF NOT EXISTS finance_sync_runs (
    ...
    created_at DATETIME(6) NOT NULL,
    PRIMARY KEY (id),
    UNIQUE KEY uq_finance_run_attempt (batch_id, platform, account_id, target_date, attempt_no),
    KEY idx_finance_runs_latest (platform, account_id, target_date, status, id),
...
CREATE TABLE IF NOT EXISTS finance_fee_items (
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uq_finance_fee_item (platform, raw_primary_fee_name, raw_secondary_fee_name, direction),
...
CREATE TABLE IF NOT EXISTS finance_summary_snapshots (
    ...
    created_at DATETIME(6) NOT NULL,
    PRIMARY KEY (id),
    UNIQUE KEY uq_finance_summary_run_fee (run_id, raw_primary_fee_name, raw_secondary_fee_name, direction),
...
CREATE TABLE IF NOT EXISTS finance_fee_mappings (
    ...
    created_at DATETIME(6) NOT NULL,
    PRIMARY KEY (id),
    UNIQUE KEY uq_finance_mapping_version (fee_item_id, effective_start_month, version_no),
    KEY idx_finance_mapping_resolve (fee_item_id, effective_start_month, effective_end_month, superseded_at),
