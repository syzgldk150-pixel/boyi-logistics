...
- AI 助手生产运行状态以 `docs/plugin-platform-v2.md` 为准：进程级 `HarnessRuntime` 直接复用当前激活的全局 `LLMClient`，状态为 `READY/ONLINE_READ_ONLY`；模型切换、回滚或清除后下一次会话立即反映新配置。助手支持自然中文对话和已注册固定只读业务查询及显式财务汇总，不依赖 Bubblewrap canary，不使用离线模拟或隐式模型降级。Session 仅保存在当前服务进程；业务查询经闭合网关最小化、脱敏并精确匹配，任何写能力均不开放。动态扩展工具仍只接受中文名称且满足 `read/compute + harness_allowed=true + broker_effect=read` 的 committed contribution。Webhook/Event 与 tracking Connector 的既有生产门禁不变。
- 自动化与插件只有一个用户入口：`/automations` 同时承载安装、升级、启停、定时、运行、设置和单实例卸载；`/extensions` 仅做 GET 重定向，旧生命周期 POST 返回 410。Service v2 安装意图只含实例名称和权限确认，安装后默认停用；插件业务设置只允许 Manifest `settings_ui.entry=settings/index.html` 的包内静态页，在无同源权限、无顶层导航/弹窗/下载/外网的 sandbox iframe 中经带会话校验的 `postMessage` 桥读写自身配置及不透明账号/资源引用。此固定设置页是“禁止插件前端”的唯一例外，不得进入 Console 主 DOM 或模块槽。插件设置 CAS 与 Console 调度 CAS 分开保存并互相保留。每个新 Service v2 包至少声明一个中文 Harness 能力；当前生产 AI 仍只执行通过治理的 `read/compute`，写贡献没有工具专属 exact preview 时保持不可用。
...
- Console 历史事项中心只能代理 Agent `/internal/v1/*`，不得直读控制平面表或恢复旧任务。管理员操作必须有真实 MySQL 会话、同源校验和服务端签名；Basic Auth 不具备写权限。飞书身份实时复核当前有效超级管理员绑定。历史审批 TTL、加锁顺序和通知唯一约束仍保留用于旧记录核验，不能产生新插件审批等待。
