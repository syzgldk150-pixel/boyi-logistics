...
- 审查修复后的离线运行状态以 `docs/plugin-platform-v2.md` 为准：进程级 `HarnessRuntime` 在真实 Bubblewrap + `prlimit` canary 成功时为 `READY/OFFLINE_RESTRICTED`，Catalog 与消息执行共用同一 active snapshot，只接受 `调用只读工具：<完整标题>` 的唯一精确零参数调用，无 handler 的六类固定工具不进有效 Catalog。Webhook/Event 只在持久注册就绪且 composition root 已绑定 `ServiceV2ManagedIngress` 时才是有效 `READY`，未绑定固定返回 `PROJECT_RUNTIME_PROJECTION_STALE`，且没有新增 HTTP 路由。tracking Connector 只能由显式 `connector-test` 开发命令调用真实 `ConnectorRegistry.invoke`，状态为 `offline_contract=COMPLETE`、`offline_runtime=READY`、`production_runtime=PRODUCTION_GATED`。真实 LLM/业务读网关、公网 Webhook/外部 Event 源、真实 Connector 和生产数据始终保持 `PRODUCTION_GATED`。
