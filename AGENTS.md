...
- 通用壳层在 `console/templates/base.html`、`console/static/style.css` 和 `console/static/console_ui.js`；Logo 资源为 `console/static/assets/boyi-logistics-logo.png`，字体资源为 `console/static/assets/fonts/SourceHanSansCN-VF.ttf.woff2` 与 `console/static/assets/fonts/InterVariable.woff2`。中文固定使用思源黑体，英文与数字固定使用 Inter，不得改回在线字体服务。响应式页面必须保留 WCAG 2.2 AA 的键盘、焦点、触控和减弱动效支持。
