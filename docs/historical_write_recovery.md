...
本组修复属于核心更新，涉及 Runner、共享仓储和 Console；历史缺范围隔离使用已有迁移 `042`，动作范围修复不新增迁移。兼容插件的业务代码与现有日常脚本入口保持原有维护归属。
...
- 旧混合任务可能把整份执行范围复制给一笔飞书写入。只有精确原 lease 的完整性摘要、回执角色／绑定摘要唯一匹配的原绑定、原资源键和唯一原物理表共同证明该笔写入目标时，读取冲突范围才缩至该物理表，并保留针对通用账号写的关联标记。原绑定可以同时包含其他入口路由；不以绑定总数代替实际写入目标核验。缺少任一证明或存在多候选／多物理目标时仍保留原范围；不使用当前配置补造历史目标，不修改 UNKNOWN、原始键、回执或 Run。
