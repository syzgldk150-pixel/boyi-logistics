...
- 旧混合任务可能把整份执行范围复制给一笔飞书写入。只有精确原 lease 的唯一资源绑定、回执角色／绑定摘要、原资源键和唯一原物理表共同证明该笔写入目标时，读取冲突范围才缩至该物理表，并保留针对通用账号写的关联标记。缺少任一证明仍保留原范围；不使用当前配置补造历史目标，不修改 UNKNOWN、原始键、回执或 Run。
