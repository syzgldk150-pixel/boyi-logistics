---
type: migration-status-snapshot
status: snapshot
updated: 2026-08-30
snapshot_at: 2026-08-25
snapshot_commit: b1be7ae
authority: non-authoritative
---

# Boyi Logistics OS V2 迁移状态快照（2026-08-25）

> 生命周期说明：本文冻结在 `b1be7ae` / 2026-08-25，只用于追溯 TASK-000 至 TASK-051 和
> OS Lite 收敛过程，不是当前仓库、当前迁移序列、当前工具数量或生产状态的权威说明。当前事实必须
> 从仓库 `docs/README.md` 指向的现行文档、实际代码、顺序迁移、测试和标准发布门禁重新核验。

更新时间：2026-08-25

状态来源：TASK-000 本地仓库只读审计、已合并 Git 记录、TASK-010 至 TASK-051 的既有实现与验证证据，以及 2026-08-25 OS Lite 收敛分支的模块生命周期、经营只读查询和完整本地质量门禁证据

基线：`main` at `b1be7ae`；本节新增能力在 `agent/os-lite-final-vision` 分支完成，合并与生产发布状态必须以 GitHub 和 ECS 验收结果为准

重要：本文件只记录有代码、迁移、测试资产或实际验证证据支持的状态。TASK-000 至 TASK-051 均未连接 ECS、未查询生产数据库、未执行生产自动化，因此本文不声明生产插件当前启用状态、生产迁移执行状态或最近运行结果。
...
| Phase 4：自动化中心 | 已完成 | TASK-030 Catalog 权威列表、TASK-031 固定命令注册、TASK-032 新增能力模板均已完成 |
| Phase 5：模块注册 | 已完成 | 在 TASK-040 至 TASK-042 只读注册基础上，已增加固定 14 模块的生命周期、审计、动态导航/页面/API 门禁和新 Command 同事务门禁 |
| Phase 6：AI Assistant | 已完成 | 确定性财务查询继续隐藏于 LLM；新增受治理的自动化运行只读工具，以及代码拥有的飞书“经营摘要/经营情况”组合入口 |
...
- 当前迁移文件编号从 `001` 到 `027`；`027` 新增固定模块生命周期表和不可变审计事件表。
...
- 生产状态：1.0.21 已安装并完成实际统计验收；“每日到货表”S 列逐单累计到货件数已由真实表格读回校验。
...
- 插件动作：同名动作
- 迁移矩阵：`RUNNABLE`
- 当前入口：飞书两步确认；Console 提供“读取候选—人工勾选—确认执行”；Scheduler 保持关闭
- 飞书 route：`builtin.split_pending_problem_upload`
- 顶层治理：`external_write`、high、LLM 不可见
- 固定文本：只接受“分批”；旧文本只提示新命令
- 稳定性修复：签名包 1.0.22 先建立兼容桥，1.0.23 再恢复 Console 人工入口。浏览器只接收候选业务信息，预览指纹仅在 Agent 服务端持久化并在确认时恢复；浏览器不能提交或替换指纹。Scheduler 保持关闭，飞书两步确认不变。
...
- 稳定性修复：签名包 1.0.22 先建立兼容桥，1.0.23 再恢复 Console 人工入口；候选由同一次只读预览产生，浏览器只提交勾选的运单号，Agent 从已验证的服务端结果恢复 64 位指纹并绑定同一次预览。飞书固定命令仍使用相同的两步确认合同。
- 编排保护：真实项目工具 `automation.self_pickup_problem_upload.run` 的计划绑定完整候选集合及指纹；空集、重复、歧义格式、超限或无效指纹均在确认或计划阶段失败关闭。旧 `self_pickup_problem_upload` 直达工具继续固定阻断。
- 执行保护：签名动作保持来源 Sheet 精确绑定、两类站点/派送方式和到齐规则，在首个写入前重读完整来源、复核指纹并完成全部目标预检；随后逐单登记并分别从问题件列表独立读回。
- 验证：定向回归 150 项和 18 个子测试、完整根测试 1612 项和 277 个子测试、完整 Agent 测试 906 项和 140 个子测试、完整 Console 测试 485 项和 175 个子测试通过；30 项环境型用例跳过；注册表、运行时导入、仓库卫生、内部 API 和 Ruff 守卫通过；Sol Advisor 全新上下文独立终审 `SHIP`。
- 生产状态：未核验
...
### TASK-030 已完成：自动化列表动态化

- 成员权威：自动化页面的项目卡只由 Agent Catalog 实例和真实持久化定时行产生；静态工作流目录继续提供已有项目的展示文案与排序，但不再在 Catalog 不可用时生成项目卡。
- 唯一覆盖：每个安全规范化的 Catalog 实例即使没有定时行也恰好显示一次；同一项目的多条定时行继续只按持久化 `automation_id` 合并，不按任务名或后缀猜测。
- 排除边界：Console 删除本地 R7 隐藏名单，只接受 Agent 本次响应中精确的 `hidden_automation_ids`。Catalog 故障时同 ID 的真实持久行保持可见并阻断，避免合法身份冲突被静默隐藏。
- 投影完整性：原始实例 ID 序列与安全规范化结果不能一一对应时，整份 Catalog 实例投影失败关闭；不显示部分列表，也不信任该响应携带的隐藏身份。
- 遗留保护：缺少或包含非法 `automation_id` 的定时行仍逐条显示为“迁移/插件缺失”，配置和运行入口均关闭。
- 验证：定向回归 44 项和 29 个子测试、根测试 1627 项和 280 个子测试、Console 完整测试 499 项和 190 个子测试通过；29 项环境型根用例跳过；四项仓库守卫、Ruff 与 diff 检查通过。
- 数据库：无迁移；未修改生产 Catalog、插件、定时、ECS 或服务状态。
...
- 没有可信客户收入维度，因此不能回答“哪个客户收入下降”。
- 客服异常详情按既有边界实时查询且不持久化历史，因此不能回答异常趋势。
- 韵达财务来源仍为 `not_launched`，不得展示、调度或参与金额汇总。
...
- 迁移 `027`、模块状态投影、经营摘要和模块管理页面的生产运行结果
...
| TASK-030 自动化列表动态化 | 已完成 | 列表成员只来自 Agent Catalog 实例与持久定时行；静态补卡和 Console 本地隐藏名单已移除，Catalog 实例投影不完整时整体失败关闭 | 无 |
| TASK-031 飞书命令注册 | 已完成 | 十个首方固定命令族收敛到只读注册表并校验命令、route 与触发工具唯一；运行仍由已提交签名项目唯一认领，扫描重启边界与既有 pending 策略保持不变 | 无 |
| TASK-032 自动化新增能力模板 | 已完成 | 建立发行范围外的纯计算示例、签名前源码/manifest/工具合同预检和发布隔离回归；不生成签名、不安装实例 | 无 |
| TASK-040 菜单注册 | 已完成 | 十四个既有入口收敛到不可变菜单注册表并保持原顺序与兼容投影；注册仅含菜单身份和展示字段，不含权限或运行状态 | 无 |
| TASK-041 权限注册 | 已完成 | 十四个菜单各自登记独立查看权限并精确复用现有 `admin/super_admin` 签名角色；未知或漂移合同关闭失败，不替代现有认证和写边界 | 无 |
| TASK-042 模块状态管理 | 已完成 | 十四个模块身份按菜单顺序登记 `code_registered` 只读源码事实；不表示启用、健康、成熟度或生产状态，不提供状态切换 | 无 |
| TASK-050 Agent查询能力 | 已完成 | 新增受治理的只读财务期间汇总；单个只读一致性事务、逐账号逐日最新终态与 passed 校验、原始 Decimal 对账和输出字段均关闭失败；账号作用域为 none，不接受 SQL/账号/任意来源；该 TASK 交付时未接入自然语言 | 无 |
| TASK-051 飞书自然语言查询 | 已完成 | `query_business_finance` 保持不向 LLM 暴露；代码按中国标准时间解析闭合期间，模糊/混合/写入/利润/未启用来源固定拒绝；仅飞书管理员绑定可提交，真实结果由专用 formatter 输出 | 无 |

## 当前收敛任务

OS Lite 收敛分支在既有 TASK-000 至 TASK-051 基线上完成以下增量：

- 固定 14 模块的代码目录、迁移 `027`、生命周期 CAS 与同事务不可变审计。
- 系统管理下的模块管理页，以及可管理模块的动态菜单、移动偏好修复、页面/API 门禁。
- Command Gateway 同 UoW 门禁；普通工具按代码所有者映射，项目化命令从当前已提交签名治理锚点解析核心工具，不复制项目 ID 映射；精确幂等回放继续返回原回执。
- `query_automation_operations` 和已绑定飞书管理员的确定性经营摘要；金额不完整时不输出金额，无可信来源的客户收入和异常趋势明确不可得。

本地证据以本分支最终 CI 等价门禁与独立只读审查为准。生产是否完成只能在合并后通过标准发布器、ECS 迁移记录、服务状态、签名接口和真实页面验收确认；源码存在不等于生产已切换。

## 快照维护规则

- 本文件已经冻结，不再作为滚动的“当前状态”账本；后续事实进入对应现行架构、模块、迁移或发布文档。
- 快照正文只保留当时已经由证据确认的事实，不按后续实现倒填或改写。
- 未连接 ECS 时，生产状态写“未核验”。
- 未运行测试时，不写“测试通过”。
