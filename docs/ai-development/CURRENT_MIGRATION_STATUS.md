...
- 稳定性修复：签名包 1.0.22 恢复 Console 人工入口。浏览器只接收候选业务信息，预览指纹仅在 Agent 服务端持久化并在确认时恢复；浏览器不能提交或替换指纹。Scheduler 保持关闭，飞书两步确认不变。
...
- 稳定性修复：签名包 1.0.22 恢复 Console 人工入口；候选由同一次只读预览产生，浏览器只提交勾选的运单号，Agent 从已验证的服务端结果恢复 64 位指纹并绑定同一次预览。飞书固定命令仍使用相同的两步确认合同。
