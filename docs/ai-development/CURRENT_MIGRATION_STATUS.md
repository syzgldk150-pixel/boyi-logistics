...
状态来源：TASK-000 本地仓库只读审计、已合并 Git 记录、TASK-010A / TASK-010C1 / TASK-010C5 / TASK-010C10 / TASK-010C11 只读核验与冻结合同、TASK-010C2 CI 证据、TASK-010C3 / TASK-010C4 本地、CI 和独立只读审查证据、TASK-010C6 本地测试证据、TASK-010C7 至 TASK-010C9 本地测试、CI 和独立只读审查证据、TASK-010C12 / TASK-010C13 本地测试、CI 和独立只读审查证据、TASK-011 至 TASK-013 本地审计、实现、测试与独立只读审查证据、TASK-020 至 TASK-022 本地审计、实现与测试证据、TASK-030 至 TASK-032 本地审计与测试证据，以及 TASK-040 至 TASK-041 本地实现与测试证据

基线：`main` at `c89a12b`

重要：本文件只记录有代码、迁移、测试资产或实际验证证据支持的状态。TASK-000 至 TASK-041 均未连接 ECS、未查询生产数据库、未执行生产自动化，因此本文不声明生产插件当前启用状态、生产迁移执行状态或最近运行结果。
...
| Phase 5：模块注册 | 进行中 | TASK-040 菜单注册、TASK-041 权限注册已完成；模块状态注册待后续独立 TASK |
...
TASK-041 已完成 Console 模块权限注册。每个既有菜单必须恰有一个稳定 `console.menu.<menu_id>.view` 权限，允许角色直接复用签名 Console principal 的公开 `admin/super_admin` 集合；权限 ID、菜单覆盖、角色和不可变性均严格校验，查询未知角色或未知权限返回拒绝。该目录当前只是静态权限注册事实，尚未接管运行时鉴权；应急 Basic Auth 不具备可签名管理员身份，因此不进入模块权限目录。现有全局路由认证、高风险写操作的超级管理员检查和菜单展示均未改变。定向回归 42 项和 8 个子测试、根测试 1636 项和 280 个子测试、Agent 完整测试 913 项和 140 个子测试、Console 完整测试 523 项和 190 个子测试通过；30 项环境型用例跳过，四项仓库守卫、Ruff 与 diff 检查通过。本 TASK 未新增角色、权限引擎、数据库或生产状态。

下一 TASK 为 `TASK-042 模块状态管理`。只注册既有模块的静态状态与只读投影，不实现安装、启停、升级、数据库持久化或生产切换，不修改菜单和权限合同。
