...
- 同实例、执行容量或实际资源暂忙时，本次已受理请求在当前进程内最多等待 30 秒，释放后继续，不要求再次提交；等待可取消，总执行容量不增加，活跃请求总量另有上限。超时、过载、停服和明确业务错误仍显式终结；已结束调用、UNKNOWN 和服务重启不自动重放。
- 不同插件可以并行；等待资源不占执行名额，不产生代次 lease 或外部写入。实际写目标仍由 Broker 统一协调，公共状态显示等待资源。
