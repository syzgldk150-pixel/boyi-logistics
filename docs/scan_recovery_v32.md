...
`sync_scan_codes` 的原扫描算法和确认预览保持不变。核心恢复链只读取原 Command 的已验证预览、原 generation 账号、原写尝试 receipt，以及外部独立发件扫描账本。
...
原 Run、Step、generation、预览摘要和每批参数摘要必须一致。缺少 journal 的历史未知写不能推测 APPLIED 的完整结果，保持显式阻断；其独立未写证明仍可按既有合同处理。

041 同时将 Runner 原准入取得的规范资源锁键随现有写 receipt 保存。该值由宿主 ContextVar 经实际执行线程进入 Broker grant，插件不能自行填写。新的写动作取得当前资源锁后，读取持久 UNKNOWN receipt，用原 `execution_keys_conflict` 比较：跨实例、账号角色或凭据的同物理 Sheet 与同账号保守范围继续等待，并归还执行槽；不冲突的已知键与无关计算继续。已取消的任务若仍有 UNKNOWN lease/receipt 也保留保护。旧未知 receipt 缺少原资源键时返回可追踪的 `UNKNOWN_WRITE_SCOPE_UNAVAILABLE`，不得猜测历史范围。只读预览可以继续，正式写必须核验后释放。
