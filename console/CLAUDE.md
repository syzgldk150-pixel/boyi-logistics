...
- 承载财务工作台（`/modules/finance`），提供 BI 总览、交易明细、费用项目绑定、异常审批、运单净额和同步记录；当前只展示和同步共享来源注册表中启用的融辉三个账号，韵达财务未上线且不计入当前失败告警，历史运行只在同步记录中保留审计；数据经 `shared.finance` 仓储读取，同步动作经 Agent `sync_finance_bills` 工具执行
