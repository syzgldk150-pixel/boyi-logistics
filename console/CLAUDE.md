...
- 插件主状态与代际协调状态合并显示：`INSTALLED/ENABLED/DISABLED + STABLE` 可启用、升级或配置；准备、等待依赖、切换、排空、未知写隔离和错误状态禁用运行。停用保留精确 CAS 入口；已停用的失败 V2 实例可经服务端核验无活跃调用、未知写及依赖占用后卸载，不能用卸载绕过未核验写入。
