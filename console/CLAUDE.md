...
- 事项详情的历史写入检查使用 `static/unknown_write_recovery.js` 和既有控制平面服务；仅超级管理员检查服务端保存证据，按精确事项/Run/lease 核验，不恢复旧任务。支持范围与核心迁移规则见 `../docs/historical_write_recovery.md`。
- 一般业务执行型 POST 提交 `/internal/v1/commands`。自动化项目手工执行是专用例外，只能调用 `/internal/v1/automation-projects/{automation_id}/invoke`，Agent 从已保存项目配置派生动作与参数；浏览器和 Console 都不得向 Agent invoke body 填工具名、账号、参数或来源。同一项目只有待 Runner 领取的已接受 Run、有效执行租约、真实运行/核验 Step 或待自动重试 Run 才返回 `AUTOMATION_ALREADY_RUNNING`；已停止且无实时执行事实的历史未知写保留事项与 Evidence，但不得阻止全新 Command。Console 保留 HTTP 409，并按真实 Run、Step 与租约分别显示排队、正在执行、正在核验或等待自动重试，刷新后不得根据历史输出或孤儿状态误报“该脚本已在运行”。客服标记已读/回复/发布/附件上传、回单同步/审核都不能直调 `/tms/*` 或第三方脚本；登录/验证码、Console 本地 OCR 和博益手工运单 CRUD 不在此范围。
...
- 原页读取及受审手工领号由 `../shared/manual_entry_contracts.py` 统一判定，Console 与 Agent 不得各自维护白名单；真实接口清单、已有任务冲突后的精确 Run 追踪及验证边界见 `../docs/original_page_read_requests.md`。
