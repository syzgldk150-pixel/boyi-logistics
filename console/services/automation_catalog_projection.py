...
from shared.plugin_management import management_for, validate_management
...
def normalize_automation_project_policy_items(value: Any) -> list[dict[str, Any]]:
    ...
    counts = Counter(str(raw.get("automation_id") or "").strip() for raw in value if isinstance(raw, dict))
    for raw in value:
...
def normalize_automation_plugin_catalog(
    ...
    package_counts = Counter(str(row.get("plugin_id") or "").strip() for row in raw_packages if isinstance(row, dict))
    instance_counts = Counter(str(row.get("automation_id") or "").strip() for row in raw_instances if isinstance(row, dict))
    ...
    for raw in raw_packages:
        ...
        if (
            ...
            invalid_plugins.add(plugin_id)
            continue
        try:
            management = validate_management(raw["management"]) if "management" in raw else management_for(plugin_id)
        except ValueError:
            invalid_plugins.add(plugin_id)
            continue
        ...
        package = {
            ...
            "settings_mode": raw.get("settings_mode", "custom" if settings_ui else "unavailable"),
        }
    ...
    for raw in raw_instances:
        ...
        try:
            management = validate_management(raw["management"]) if "management" in raw else package.get("management", management_for(plugin_id))
            if plugin_id in invalid_plugins or instance_counts[automation_id] != 1 or management != package.get("management", management):
                raise ValueError("ambiguous or inconsistent catalog identity")
        except ValueError:
            if AUTOMATION_PROJECT_ID_RE.fullmatch(automation_id):
                invalid_instances.append(automation_id)
            continue
        ...
        for role_definition in resource_roles:
            ...
            blocked_reason = ""
            if value.get("resources_deferred") is True:
                # The signed catalog carries readiness; opening settings loads candidates.
                selected_available = bool(selected_resource_id)
                if bool(role_definition.get("required")) and not selected_resource_id:
                    blocked_reason = f"请选择{role_definition['label']}"
            elif not resource_pool_available and not options and (selected_resource_id or role_definition.get("required")):
                blocked_reason = (
    ...
    return packages, instances, list(dict.fromkeys(unsupported + invalid_instances))
