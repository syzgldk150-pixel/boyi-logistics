...
- 新调用不创建项目卡待审批队列。旧待审批集合仅为历史只读兼容；不得用旧审批数量或孤儿 Run 状态阻止新的插件调用。
...
- 自动化侧边栏点击后必须立即更新选中态并显示可访问骨架，不等待 Agent、账号、权限或飞书请求完成。部分导航允许在 60 秒新鲜期后继续使用最多 300 秒的安全 Catalog 快照，账号和权限外部读取限时 3 秒；真实页面完成后替换骨架。折叠卡片使用延迟渲染，页面隐藏时运行轮询至少退避到 5 秒。安装、升级、启停、卸载和显式资源刷新仍必须立即清缓存。卡片只按当前 Invocation 的真实启动、执行、核验和终态显示；资源不足或同实例仍在运行时明确拒绝，不新增排队状态或把未启动显示成已执行。插件卡片不得恢复“任务怎么运行”“允许从哪里启动”“数据从哪里读取、保存到哪里”等统一业务表单；插件业务字段只在该实例设置页出现；账号引用和平铺常用参数可使用宿主简单设置，Console 卡片只保留执行、取消、启停、定时、权限、状态、输出、升级和卸载。
...
- Service v2 业务设置不再由自动化卡片统一渲染账号、入口、数据来源或保存位置。Manifest 可选声明固定 `settings_ui.entry=settings/index.html` 和 `bridge_api=1.0.0`；账号引用和平铺常用参数可使用宿主简单设置页；复杂必填配置或资源角色仍需专属设置页。点击“设置”进入 `/automations/{automation_id}/settings`，包内页面运行在无 `allow-same-origin` 的 sandbox iframe，CSP 禁止外网、顶层跳转、弹窗、下载和非包内资源，只能经会话绑定、来源校验的 `postMessage` 桥读取自身配置、脱敏账号状态、脱敏飞书目录并保存不透明引用。插件设置 CAS 只更新配置/绑定且保留定时；Console 调度 CAS 只更新时间计划且保留插件设置。
...
- 插件主状态与代际协调状态合并显示：`INSTALLED/ENABLED/DISABLED + STABLE` 可启用、升级或配置；准备、等待依赖、切换、排空、未知写隔离和错误状态禁用运行。停用保留精确 CAS 入口；已停用的失败 V2 实例可经服务端核验无活跃调用、未知写及依赖占用后卸载，不能用卸载绕过未核验写入。 已撤回或无迁移的 V2 在卸载撤销完成且协调状态 STABLE 后可重试卸载；仍走原 CAS、受保护写检查及精确清理入口，排空中与未知写状态不开放重试。
...
- 后台账号页允许当前 Console 超级管理员创建/撤销飞书审批绑定码；页面不得展示 `open_id/chat_id`。绑定后飞书角色实时继承账号当前 `control_plane_role/is_active`；旧审批通知/1/2 协议只保留历史核验，不为新插件创建审批队列。
...
- 改统一回单管理页：
  ...
  - `app.py`
  - `database.py`
  - 页面入口 `/receipts` 使用本地 `receipt_records/receipt_attachments/receipt_audit_logs`。查询走普通 receipts-query，审核走 receipts-audit 直接业务动作；独立读回核验成功后才更新本地审核状态，未知结果不得伪造成功。不创建旧 Command/Run 或隐式写 iframe。
- 改财务工作台：
  - `templates/base.html`
  ...
  - 财务查询经共享账本直接读取。同步、回填和重试经 `/internal/v1/business/finance-collect` 转到用户明确选择的已安装财务采集插件；后台显示对应 Invocation。真实管理员、同源、稳定浏览器 UUID 和精确账号绑定均保留，不创建事项审批或 Command/Run；不传凭据。
  ...
  - 财务采集完成不自动分析；显式分析才调用已注册分析接口，读取共享账本。Console 不另启模型或通过旧 execute_tool/Run 完成事件重复执行；模型设置、测试、reload 仍要求签名管理员。
- 改客服系统问题件工作台：
  - `templates/base.html`
...
- 热敏主单使用用户桌面“主单模板/主单-空白信息版.jpg”的新版原始底图，保存为 `static/assets/waybill_label_background.jpg`，原图 1162 × 1450 px，纸张仍为 74mm × 92mm；以同目录“主单.jpg”为字号、粗细和位置参考。`static/js/waybill_label_html.js` 单点维护分区坐标、各字段字号、数据归一化和排版，`static/js/waybill_label_lodop.js` 复用同一张排好内容的高分辨率 PNG，通过 `ADD_PRINT_IMAGE` 输出；C-Lodop 不再二次折行主单内容，避免单号、日期、电话截断与地址叠字。原始底图直接加载，不重绘。动态内容使用本机黑体并加粗：单号、目的地和姓名突出，电话、地址、货物和费用分别按样张大小排版；各字段接在固定标签后左对齐，同排文字保持对齐，地址和备注从顶部换行。浏览器加载本机黑体（SimHei）后按 700 字重测量，浏览器 Canvas 使用相同黑体和字重生成最终面单；缺字体明确报错，长内容按区域缩小或换行，超出区域明确报错，不截断。录单页“需要回单”复选框保存为 `boyi_waybills.receipt_required`（迁移 `052_boyi_waybill_receipt_required.sql`）；勾选使用 `static/assets/waybill_label_receipt_background.jpg`，未勾选和历史单使用空白框底图，预览、保存后打印及补打共用选择逻辑。此打印选项不进入插件的闭合录单草稿协议；新版不再包含制单人、经办人或签收栏，旧坐标和字段已移除。重量/体积按保存的 kg / m³ 分栏；费用和备注使用实际值，空值不补零，发件地址无来源时留空，不使用样张人物或金额。图片与脚本版本参数同步更新；禁止重绘底图、旧模板兜底、`ADD_PRINT_HTM` 或浏览器打印兜底。
...
- 15 个固定模块不再由旧数据库生命周期状态、版本或 Agent 可达性控制；导航、页面、API 与新调用 只依赖代码注册、既有登录/角色权限和各业务自身前置条件。不得重新引入生命周期状态门禁，也不得改变现有模块权限。
...
- 插件专属设置页必须先安装宿主消息监听器，再给隔离 iframe 设置 `src`，防止缓存子页的首次配置请求丢失；时序回归见 `tests/test_plugin_settings_initialization.py`，真实保存／改绑／历史恢复验收见 `../tests/v32_acceptance/custom_settings.py`。
