...
- 热敏主单使用用户桌面“主单模板/主单-空白信息版.jpg”的新版原始底图，保存为 `static/assets/waybill_label_background.jpg`，原图 1162 × 1450 px，纸张仍为 74mm × 92mm；以同目录“主单.jpg”为字号、粗细和位置参考。`static/js/waybill_label_html.js` 单点维护分区坐标、各字段字号、数据归一化和排版，`static/js/waybill_label_lodop.js` 复用后先 `ADD_PRINT_IMAGE` 再 `ADD_PRINT_TEXT`。动态内容使用本机黑体并加粗：单号、目的地和姓名突出，电话、地址、货物和费用分别按样张大小排版；各字段接在固定标签后左对齐，同排文字保持对齐，地址和备注从顶部换行。浏览器加载本机黑体（SimHei）后按 700 字重测量，C-Lodop 使用“黑体”并启用加粗；缺字体明确报错，长内容按区域缩小或换行，超出区域明确报错，不截断。底图的签回单框保留空白，当前无对应数据源；新版不再包含制单人、经办人或签收栏，旧坐标和字段已移除。重量/体积按保存的 kg / m³ 分栏；费用和备注使用实际值，空值不补零，发件地址无来源时留空，不使用样张人物或金额。图片与脚本版本参数同步更新；禁止重绘底图、旧模板兜底、`ADD_PRINT_HTM` 或浏览器打印兜底。
