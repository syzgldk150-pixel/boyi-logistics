...
def handle_get(app: Any, handler: Any, path: str, raw_path: str, query: dict[str, list[str]]) -> bool:
    ...
    if path == "/ocr/boyi/frame":
        ...
        return True
    if raw_path.startswith("/ocr/ronghui/live"):
        app._handle_ronghui_live_proxy(handler, raw_path, method="GET", query=query)
        return True
    if raw_path.startswith("/ocr/yunda/live"):
        app._handle_yunda_live_proxy(handler, raw_path, method="GET", query=query)
        return True
    return False
...
def handle_post(app: Any, handler: Any, path: str, raw_path: str, query: dict[str, list[str]]) -> bool:
    if raw_path.startswith("/ocr/ronghui/live"):
        app._handle_ronghui_live_proxy(handler, raw_path, method="POST", query=query)
        return True
    if raw_path.startswith("/ocr/yunda/live"):
        app._handle_yunda_live_proxy(handler, raw_path, method="POST", query=query)
        return True
...
def handle_write(
    ...
    if raw_path.startswith("/ocr/ronghui/live"):
        app._handle_ronghui_live_proxy(handler, raw_path, method=method, query=query)
        return True
    if raw_path.startswith("/ocr/yunda/live"):
        app._handle_yunda_live_proxy(handler, raw_path, method=method, query=query)
        return True
    return False
